/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Filename: txSignedWSMsWithCallbacks.c                                      */
...
/*----------------------------------------------------------------------------*/
/*
 * Transmits 10 signed WSM's per seconds with callbacks. Therefore,
...
int init(void)
{
    ...
    risRet = wsuRisInitRadioComm("txSignedWSMsWithCallbacks", &appCredential);
    ...
    init.debug                             = FALSE;
    init.sec_verify_approach               = smi_verify_on_demand;
...
int main(int argc, char **argv)
{
    int              retVal = 0;
    rsResultCodeType risRet;
    uint8_t          payload[] = "Tx signed WSMs with callbacks";
    int              txCount = 0;
    cfgType          getCfg;
    ...
    while (mainLoop) {
        ...
        if (txCount % 10 == 0) {
            printf("Packets transmitted = %u; sign count = %u\n",
                   txCount, signCount);
        }
