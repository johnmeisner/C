/*----------------------------------------------------------------------------*/
/*                                                                            */
/* Filename: rxSignedWSMsWithVerifyVOD.c                                      */
...
/*----------------------------------------------------------------------------*/
/*
 * Receives signed WSM's with verification once a second via VOD. Calls to
 * smiInitialize() and smiTerm() are required.
 */
...
int init(void)
{
    ...
    risRet = wsuRisInitRadioComm("rxSignedWSMsWithVerifyVOD", &appCredential);
    ...
    init.vtp_milliseconds_per_verification = 65535;
...
int main(int argc, char **argv)
{
    ...
    while (mainLoop) {
        ...
        if ((count >= 10) && (vodMsgSeqNum != 0)) {
            count = 0;
            localVodMsgSeqNum = vodMsgSeqNum;
            vodMsgSeqNum = 0;

            if (localVodMsgSeqNum != 0) {
