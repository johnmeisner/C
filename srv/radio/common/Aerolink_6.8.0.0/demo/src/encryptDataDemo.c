...
int main (int argc, char *argv[])
{
    ...
    for( i = 1; i < argc; ++i )
    {
        ...
        else if( strcmp("-payload", argv[i]) == 0 )
        {
            if (i+1 < argc)
            {
                ...
                {
                    memcpy(plainText, argv[1+1], size);
                    plainTextLength = size;
                    i++;
                }
                else
                {
                    printf("Invalid argument: length of payload must be less than %zu\n", sizeof (plainText));
                }

            }
            else
            {
                printf("Invalid argument: -payload requires a string value, the payload to be encrypted\n");
                return 1;
            }
        }
    ...
    result = securityServices_generatePublicEncryptionKeyPair(PEA_ECIES_NISTP256, &encryptionKey);
    if (result != WS_SUCCESS)
    {
