...
#include <time.h>           //for clocksettime()
#include "timepps.h"
...
int fetch_source(pps_handle_t handle, int avail_mode)
{
	...
	if (div > 1000)
	{
	    printf("-------SetTime---------delta=%6.2f\n", delta);

        ///static struct timespec setts;
        ///setts.tv_sec = ts.tv_sec;
	    ///setts.tv_nsec = 0;
	    ///clock_settime(CLOCK_MONOTONIC, &setts);

	    static struct timeval settv;
	    settv.tv_sec = ts.tv_sec;
	    settv.tv_usec = 1000;
