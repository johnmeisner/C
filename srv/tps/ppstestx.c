...
int fetch_source(int i, pps_handle_t *handle, int *avail_mode)
{
	...
	static struct timeval  systv;
	static struct timespec systs;

    gettimeofday(&systv, NULL);
    clock_gettime(CLOCK_MONOTONIC, &systs);
...
retry:
    ...
    int deltatime;
	deltatime = abs(infobuf.assert_timestamp.tv_sec - systs.tv_sec);
    printf("  deltatime=%d, ",deltatime);


	if (abs(infobuf.assert_timestamp.tv_sec - systs.tv_sec) > 5)
	{
	    struct timeval ppstv;

	    ppstv.tv_sec  = infobuf.assert_timestamp.tv_sec;
	    ppstv.tv_usec = infobuf.assert_timestamp.tv_nsec * 1000;
	    printf("--->Set SysTime\n");
	    settimeofday(&ppstv, NULL);
	}
	else
	{
	    printf("\n");
