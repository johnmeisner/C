...
class EXTPERCLASS ASN1PERMessageBuffer : public ASN1MessageBuffer {
 ...
 public:
   ...
   /**
    * This method sets the bit count in the last created field.  The count
    * is calculated by subtracting the current bit offset from the offset
    * that was saved after the last newBitField call.
    */
...
// ASN.1 PER encoded message info
class EXTPERCLASS ASN1PEREncInfo {
 public:
   size_t mLenBitOffset;  // Bit offset to encoded length
   size_t mLenNumBits;    // Number of bits in encoded length
   size_t mDataBitOffset; // Bit offset to encoded data
   size_t mDataNumBits;   // Number of bits in encoded data

   ASN1PEREncInfo(): mLenBitOffset(0), mLenNumBits(0), mDataBitOffset(0),
      mDataNumBits(0) {}
} ;
...
class EXTPERCLASS ASN1PEREncodeBuffer : public ASN1PERMessageBuffer {
 public:
    ...
    * @param pSizeCnst  Pointer to size constraint structure.
    * @return           Number of bytes encoded or negative status code if
    *                     error occurred. Note that returned encoded length
    *                     may be less than length value if fragmented length.
    */
    ...
    *
    * @param numItems   Number of items to be encoded (bits for BIT STRING
    *                     or octets for OCTET STRING).
    * @param pdata      Pointer to byte array containing data to be encoded.
    * @param pSizeCnst  Pointer to size constraint structure.
    * @param bitStr     Flag indicating BIT STRING is being encoded (false
    *                     if OCTET STRING).
    * @param pEncInfo   Pointer to structure to receive information on the
    *                     encoding (length and data bit counts and offsets).
    * @return           Status of operation: 0 = success, negative
    *                     value if error occurred.
    */
   int encodeBitOrOctStr
      (size_t numItems, const OSOCTET* pdata, Asn1SizeCnst* pSizeCnst,
       OSBOOL bitStr, ASN1PEREncInfo* pPEREncInfo = 0);
 ...
 public:
   ...
   ASN1PERDecodeBuffer (const OSOCTET* pMsgBuf,
      ...
      ASN1PERMessageBuffer
         (PERDecode, (OSOCTET*)pMsgBuf, msgBufLen, aligned, pContext) {}
   ...
   /**
    * This method decodes the given number of bits (up to 32) from
    * the input buffer/stream into an unsigned 32-bit integer
    * variable.
    *
    * @param nbits Number of bits to decode
    * @param value Reference to unsigned integer variable to receive
    *              decoded value.
    * @return      Status of operation: 0 = success, negative
    *              value if error occurred.
    */
   ...
   /**
    * This method decodes the given number of bits from the input
    * buffer/stream into the given byte array.
    *
    * @param nbits  Number of bits to decode
    * @param buffer Reference to byte array to received decoded value.
   ...
   /**
    * This method decodes the given number of whole bytes from the input
    * buffer/stream into the given byte array.  The input buffer/stream
    * is assumed to be aligned to a byte boundary.
    *
