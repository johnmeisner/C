...
#ifdef _TRACE
   *ppPerField = (pctxt->pBitFldList) ? pctxt->pBitFldList->fieldList.tail : 0;
#else
   *ppPerField = 0;
#endif
   ...
   OS_UNUSED_ARG(pPerField);
         ...
         */
         ...
         if (0 != pctxt->pBitFldList) {
            OSRTSListNode* pNode = (OSRTSListNode*) pPerField;
            OSRTSList* pList = &pctxt->pBitFldList->fieldList;
            PERField* pField = 0;
            size_t border = 0;
            size_t offset = 0;
               ...
               while (nm--) {
                  ...
                  if (pNode) {
                     if (pField->bitOffset < border) {
                        /* split current field */
                        size_t nameSz = OSCRTLSTRLEN (pField->elemName);
                        ...
                        name = (char *) rtxMemAlloc (pctxt, nameSz + 5);
                              /* +5 allows for null terminator and ' ...' */
                        ...
                        rtxStrcpy (name, nameSz+5,  pField->elemName);

                        if (nameSz >= 4 &&
                            pField->elemName[nameSz - 1] != '.' &&
                            pField->elemName[nameSz - 2] != '.' &&
                            pField->elemName[nameSz - 3] != '.' &&
                            pField->elemName[nameSz - 4] != ' ')
                           name = rtxStrcpy (name + nameSz, 5, " ...");
                  ...
                  name = (char *) rtxMemAlloc (pctxt, 15);
                  if (name == 0) {
                     rtxMemFreePtr (pctxt, ptm);
                     rtxMemFreePtr (pctxt, pnd);
                     return LOG_RTERR (pctxt, RTERR_NOMEM);
                  ...
                  name = rtxStrdup (pctxt, " segment length");
