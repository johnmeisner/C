...
EXTPERMETHOD int pd_Length64 (OSCTXT* pctxt, OSSIZE* pvalue)
{
   ...
   if (upper < OSINTCONST(65536) && !extbit) {
...
{
   if (pctxt->buffer.aligned) {
      int stat = PD_BYTE_ALIGN (pctxt);
      if (stat != 0) return LOG_RTERR (pctxt, stat);
   }
   return pd_UnconsLen64Value(pctxt, pvalue);
}

EXTPERMETHOD int pd_UnconsLen64Value (OSCTXT* pctxt, OSSIZE* pvalue)
{
   int stat = 0;
   OSUINT32 u32value;
   OSBOOL bitValue;
   ...
   if (bitValue == 0) {
      ...
      if (stat != 0) return LOG_RTERR (pctxt, stat);
      else if (pvalue) *pvalue = u32value;
   }
   else {
      stat = PD_BIT (pctxt, &bitValue);
      if (stat != 0) return LOG_RTERR (pctxt, stat);
...
EXTPERMETHOD int pd_ConsLength
(OSCTXT* pctxt, OSUINT32* pvalue, OSUINT32 rangeBitCount, OSUINT32 lowerBound)
{
   int ret = rtxDecBits(pctxt, pvalue, rangeBitCount);
   if (0 == ret) {
      if (pvalue)
         *pvalue += lowerBound;
      return 0;
...
EXTPERMETHOD int pd_ConsLength64
(OSCTXT* pctxt, OSSIZE* pvalue, OSSIZE rangeBitCount, OSSIZE lowerBound)
{
   int ret = rtxDecBitsToSize(pctxt, pvalue, rangeBitCount);
   if (0 == ret) {
      if (pvalue)
         *pvalue += lowerBound;
      return 0;
