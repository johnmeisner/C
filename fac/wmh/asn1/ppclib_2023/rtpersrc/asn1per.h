...
/* The following constant defines the maximum number of bytes permitted */
/* for a big integer.                                                   */
...
#ifdef _TRACE
#define PU_INSLENFLD(pctxt)       RTDIAG_INSBITFLDLEN(pctxt)
#define PU_NEWFIELD(pctxt,suffix) RTDIAG_NEWBITFIELD(pctxt,suffix)
#define PU_PUSHNAME(pctxt,name)
#define PU_PUSHELEMNAME(pctxt,idx)
#define PU_POPNAME(pctxt)
#define PU_SETBITOFFSET(pctxt)    RTDIAG_SETBITFLDOFFSET(pctxt)
#define PU_SETBITCOUNT(pctxt)     RTDIAG_SETBITFLDCOUNT(pctxt)
#define PU_SETOPENTYPEFLDLIST(pMainBFList,pOpenTypeBFList) \
pu_setRtxDiagOpenTypeFldList(pMainBFList,pOpenTypeBFList)
#else
 ...
 *   -# Call the pu_newContext or the rtInitContext and pu_setBuffer functions
*  to specify the address of the buffer containing the encoded ASN.1 data to be
* decoded and whether the data is aligned, or unaligned.
...
/**
 * 64-bit version of pd_BitString.  Length is returned in a size-typed
 * variable which scales to 64-bits on 64-bit machines.
 *
 * @see pd_BitString
 */
EXTERNPER int pd_BitString64
(OSCTXT* pctxt, OSSIZE* numbits_p, OSOCTET* buffer, OSSIZE bufsiz);

/**
 * This version of pd_BitString will decode a bit string into the standard
 * 32-bit bit string type which can hold up to 32 bits.
 *
 * @see pd_BitString
 */
EXTERNPER int pd_BitString32
(OSCTXT* pctxt, ASN1BitStr32* pbitstr, OSSIZE lower, OSSIZE upper);
 ...
 *                       values.  Pass NULL to discard the decoded value.
...
(OSCTXT* pctxt, OSINT32* pvalue, OSINT64 lower, OSINT64 upper);
...
(OSCTXT* pctxt, OSINT8* pvalue, OSINT64 lower, OSINT64 upper);
...
EXTERNPER int pd_ConsInt16 (OSCTXT* pctxt,
                            OSINT16* pvalue, OSINT64 lower, OSINT64 upper);
...
/**
 * This function will decode a constrained length value. In this case, the
 * range bit count is pre-calculated.
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue      Pointer to 32-bit unsigned integer variable to receive
 *                       decoded value.
 * @param rangeBitCount Bit count of the value range.
 * @param lowerBound  Lower bound of the value constraint.
 * @return            Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_ConsLength
(OSCTXT* pctxt, OSUINT32* pvalue, OSUINT32 rangeBitCount, OSUINT32 lowerBound);

EXTERNPER int pd_ConsLength64
(OSCTXT* pctxt, OSSIZE* pvalue, OSSIZE rangeBitCount, OSSIZE lowerBound);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_ConsUnsignedSignedBound
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_ConsUInt8SignedBound (OSCTXT* pctxt,
...
(OSCTXT* pctxt, OSUINT16* pvalue, OSUINT64 lower, OSUINT64 upper);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_ConsUInt16SignedBound
(OSCTXT* pctxt, OSUINT16* pvalue, OSINT64 lower, OSINT64 upper);
...
EXTERNPER int pd_ConsUInt64 (OSCTXT* pctxt, OSUINT64* pvalue,
   ...
   #define pd_ConsSize(pctxt, pvalue, lower, upper) \
      pd_ConsUnsigned(pctxt, pvalue, lower, upper)
#elif defined OSSIZE_IS_64BITS
   #define pd_ConsSize(pctxt, pvalue, lower, upper) \
      pd_ConsUInt64(pctxt, pvalue, lower, upper)
#endif
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_ConsUInt64SignedBound (OSCTXT* pctxt, OSUINT64* pvalue,
                             OSINT64 lower, OSINT64 upper);

#if defined OSSIZE_IS_32BITS
   #define pd_ConsSizeSignedBound(pctxt, pvalue, lower, upper) \
      pd_ConsUnsignedSignedBound(pctxt, pvalue, lower, upper)
#elif defined OSSIZE_IS_64BITS
   #define pd_ConsSizeSignedBound(pctxt, pvalue, lower, upper) \
      pd_ConsUInt64SignedBound(pctxt, pvalue, lower, upper)
#endif
...
/**
 * This function decodes constrained string data into a fixed-size
 * buffer (i.e. character array).  The buffer size must be at least one byte
 * breater than the length being decoded.
 *
 * @param pctxt       Pointer to context block structure.
 ...
 * @param nbits       Number of bits in a character set character.
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_ConstrainedStringData
(OSCTXT* pctxt, char* strbuf, OSSIZE bufsize, OSSIZE len,
 const char* charSet, OSSIZE nbits,  OSSIZE canSetBits);

/**
 * This function decodes constrained string data into a dynamic
 * character string buffer,  Memory is allocated for the string using
 * the rtxMemAlloc function.  It is freed by calling rtxMemFreePtr to
 * with this pointer or by calling rtxMemFree to free all memory held
 * by the context.
 *
 * @param pctxt       Pointer to context block structure.
 * @param ppstr       Pointer to character string to receive decoded string.
 * @param len         Number of characters to decode.
 * @param charSet     String containing permitted alphabet character set. Can
 *                      be null if no character set was specified.
 * @param nbits       Number of bits in a character set character.
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_DynConstrainedStringData
 ...
 *                      Pass NULL to discard the decoded value.
 ...
 *                      in string form (YYYY:MM:DD) or NULL to discard.
...
/**
 * This function will decode an ISO 8601 DATE type into a preallocated
 * character string buffer.
 *
 * @param pctxt       Pointer to context block structure.
 * @param string      Pointer to buffer to receive decoded value
 *                      in string form (YYYY:MM:DD) or NULL to discard.
 * @param strSz       Size of string buffer.
 * @param flags       Set of flags: OSANY|OSCENTURY|OSYEAR|OSMONTH|OSWEEK|OSDAY.
 * @return             Completion status of operation:
 *                       - >= 0: success; indicates the number of characters
 *                          put into string (0 if string is NULL)
 *                       - negative return value is error.
 */
EXTERNPER int
pd_DateToStrn (OSCTXT* pctxt, char* string, OSSIZE strSz, OSUINT32 flags);
 ...
 * @param pctxt       Pointer to context block structure.
 * @param string      Pointer to string variable to receive decoded value
 *                      in string form (YYYY-MM-DDTHH:MM:SS) or NULL to discard.
 * @param flags       Set of flags: OSANY|OSCENTURY|OSYEAR|OSMONTH|OSWEEK|OSDAY|
 ...
 * @param pctxt       Pointer to context block structure.
 * @param string      Pointer to string variable to receive decoded value
 *                      in string form (PnYnMnDTnHnMnS) or NULL to discard.
...
/**
 * This function will decode an ISO 8601 DURATION type into a preallocated
 * character string buffer.
 *
 * @param pctxt       Pointer to context block structure.
 * @param string      Pointer to string variable to receive decoded value
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_DurationToStrn (OSCTXT* pctxt, char* string, size_t strSz);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_DynBitString (OSCTXT* pctxt, ASN1DynBitStr* pBitStr);
 ...
 * @see pd_DynBitString
 */
EXTERNPER int pd_DynBitString64 (OSCTXT* pctxt, ASN1DynBitStr64* pBitStr);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_DynOctetString (OSCTXT* pctxt, ASN1DynOctStr* pOctStr);

/**
 * 64-bit version of pd_DynOctetString.
 *
 * @see pd_DynOctetString
 */
EXTERNPER int pd_DynOctetString64 (OSCTXT* pctxt, OSDynOctStr64* pOctStr);

/**
 * This function decodes a value of the ASN.1 object identifier type.
 * This version of the function decodes into a structure have a dynamic
 * array to hold the decoded subidentifiers.
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue      Pointer to value to receive decoded result. The
 *                      ASN1DynOBJID structure contains an integer to hold the
 *                      number of subidentifiers and a dynamic array to hold
 *                      the subidentifier values. Memory for the array is
 *                      allocated using the rtxMemAlloc function.
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_DynObjId (OSCTXT* pctxt, ASN1DynOBJID* pvalue);

/**
 * This function decodes the optional bit field at the beginning of
 * a SEQUENCE or SET extension block.
 *
 * @param pctxt       A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
 *                       calls.
 * @param poctstr     Pointer to a dynamic OCTET STRING structure to hold
 *                       the decoded optional bit values. The decoded bits
 *                       are stored as an array of boolean values in the
 *                       data field. Memory for this field is allocated and
 *                       must be released after decoding the extension
 *                       elements is complete.
 * @return            Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_ExtOptElemBits(OSCTXT* pctxt, OSDynOctStr* poctstr);
 ...
 * This function gets the total length of a PER-encoded component.  In
 * the case of a fragmented length, it will look ahead and add up each
 * of the individual length components.  This version will return length
 * value up to 64-bits in size in 64-bit systems.
 *
 * @param pctxt       A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
 *                       calls.
 * @param itemBits     The size of the specific entity.
 * @param plength      Pointer to variable to hold parsed length value.
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_GetComponentLength64
...
/**
 * This function will decode a length determinant value.  This variant
 * support lengths up to 64 bits in size on 64-bit architectures.
 *
 * @param pctxt       A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
 *                       calls.
 * @param pvalue       A pointer to a size type variable to receive the
 *                       decoded length value.
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_Length64 (OSCTXT* pctxt, OSSIZE* pvalue);
 ...
 * maximun size is known in advance. The ASN1C compiler generates a call to
 * this function to decode octet string productions or elements that contain a
 * size constraint.  This varaint of the function supports OCTET STRING's
 * with sizes up to 64 bits in size on 64 bit systems.
 *
 * @param pctxt       Pointer to a context structure. This provides a storage
 *                       area for the function to store all working variables
 *                       that must be maintained between function calls.
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_OctetString64
 ...
 * This function allows non-zero finite REAL values to be encoded in base 2 or
 * base 10.
...
/**
 * This function will decode a value of the binary encoded ASN.1 real type.
 * This function requires that non-zero finite REAL values be encoded in base 2.
 * This function provides support for the plus-infinity special real values.
 *
 * @param pctxt       Pointer to a context structure. This provides a storage
 *                       area for the function to store all workings variables
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_Real2 (OSCTXT* pctxt, OSREAL* pvalue);
...
EXTERNPER int pd_SemiConsInteger
   (OSCTXT* pctxt, OSINT32* pvalue, OSINT64 lower);
...
EXTERNPER int pd_SemiConsUnsigned
   (OSCTXT* pctxt, OSUINT32* pvalue, OSUINT64 lower);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsUnsignedSignedBound
   (OSCTXT* pctxt, OSINT32* pvalue, OSINT64 lower);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsInt8
   (OSCTXT* pctxt, OSINT8* pvalue, OSINT64 lower);

/**
 * This function will decode a semi-constrained unsigned 8-bit integer.
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue       Pointer to unsigned 8-bit integer variable to receive
 *                       decoded value.
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsUInt8
   (OSCTXT* pctxt, OSUINT8* pvalue, OSUINT64 lower);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsUInt8SignedBound
   (OSCTXT* pctxt, OSUINT8* pvalue, OSINT64 lower);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsInt16
   (OSCTXT* pctxt, OSINT16* pvalue, OSINT64 lower);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsUInt16
   (OSCTXT* pctxt, OSUINT16* pvalue, OSUINT64 lower);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsUInt16SignedBound
   (OSCTXT* pctxt, OSUINT16* pvalue, OSINT64 lower);
...
#if defined OSSIZE_IS_32BITS
   #define pd_SemiConsSize(pctxt, pvalue, lower) \
      pd_SemiConsUnsigned(pctxt, pvalue, lower)
#elif defined OSSIZE_IS_64BITS
   #define pd_SemiConsSize(pctxt, pvalue, lower) \
      pd_SemiConsUInt64(pctxt, pvalue, lower)
#endif
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_SemiConsUInt64SignedBound
   (OSCTXT* pctxt, OSUINT64* pvalue, OSINT64 lower);

/**
 * This function converts a time difference to a string fragment.
 *
 * @param pctxt       Pointer to context block structure.
 * @param string      Pointer to buffer to receive string value.
 * @param strSz       Size of string buffer.
 * @return  Number of characters in converted string or negative status.
 */
EXTERNPER int pd_TimeDiffToStrn(OSCTXT* pctxt, char* string, OSSIZE strSz);
 ...
 * @param pctxt       Pointer to context block structure.
 * @param string      Pointer to string variable to receive decoded value
 *                      in string form (HH:MM:SS) or NULL to discard.
...
/**
 * This function will decode ISO 8601 TIME types into a preallocated
 * character string buffer.
 *
 * @param pctxt       Pointer to context block structure.
 * @param string      Pointer to string variable to receive decoded value
 *                      in string form (HH:MM:SS) and etc.
 * @param strSz       Size of string buffer.
 * @param flags       Set of flags: OSHOURS|OSMINUTES|OSSECONDS|OSUTC|OSDIFF|n.
 *                      n - set digit number of fraction part.
 * @return             Completion status of operation:
 ...
 */
EXTERNPER int
pd_TimeToStrn (OSCTXT* pctxt, char* string, size_t strSz, OSUINT32 flags);
...
/**
 * This function will decode an unconstrained length value or a length
 * value with upper bound > 64k.  This variant supports lengths up to
 * 64 bits in size. This will do alignment if required prior to decoding
 * the value.
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue      Pointer to size type variable to receive decoded value.
 * @return            Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_UnconsLength64 (OSCTXT* pctxt, OSSIZE* pvalue);

/**
 * This function will decode an unconstrained length value or a length
 * value with upper bound > 64k.  This variant supports lengths up to
 * 64 bits in size and assumes the buffer is already aligned
 * prior to decoding.
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue      Pointer to size type variable to receive decoded value.
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_UnconsLen64Value (OSCTXT* pctxt, OSSIZE* pvalue);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_UnconsInt8 (OSCTXT *pctxt, OSINT8* pvalue);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_UnconsUInt8 (OSCTXT* pctxt, OSUINT8* pvalue);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pd_UnconsInt16 (OSCTXT *pctxt, OSINT16* pvalue);
 ...
 *
 * @param pctxt       Pointer to context block structure.
 * @param pvalue       Pointer to unsigned 16-bit integer variable to receive
 *                       decoded value.
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pd_UnconsUInt16 (OSCTXT* pctxt, OSUINT16* pvalue);
...
#if defined OSSIZE_IS_32BITS
   #define pd_UnconsSize(pctxt, pvalue) pd_UnconsUnsigned(pctxt, pvalue)
#elif defined OSSIZE_IS_64BITS
   #define pd_UnconsSize(pctxt, pvalue) pd_UnconsUInt64(pctxt, pvalue)
#endif
...
EXTERNPER int pe_bitsAligned (OSCTXT* pctxt, OSUINT32 value, OSUINT32 nbits);
 ...
 *                       variables that must be maintained between function
 *                       calls.
 * @param numbits      The number of bits in the string to be encoded.
...
/* Internal function used to encode a binary string segment for BIT or
   OCTET STRING.  Allows length encoding to be separated from content. */
...
/**
 * This function will encode a value of the ASN.1 bit string type.
 * In this case, the bit string has an extended size and may contain
 * extension data.
 *
 * @param pctxt       A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
 *                       calls.
 * @param numbits      The total number of bits in the string to be encoded
 *                       (root + extension).
 * @param data         Pointer to the root bit string data to be encoded.
 * @param dataSize     Size, in octets, of the root bit string data.
 * @param extData      Pointer to extension bit string data to be encoded.
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTPERMETHOD int pe_BitStringExt
(OSCTXT* pctxt, OSSIZE numbits, const OSOCTET* data, OSSIZE dataSize,
 ...
 *                       - negative return value is error.
 */
#define pe_ConsInteger(pctxt, value, lower, upper) \
   pe_ConsInt64(pctxt, value, lower, upper)
...
/**
 * This function encodes constrained string data. It is assumed that
 * the length of the string was previously encoded and alignment, if
 * necessary, was done.
 *
 * @param pctxt        Pointer to context block structure.
 * @param string       Pointer to string to be encoded.
 ...
 * @param canSetBits   Number of bits in a character from the canonical set
 *                       representing this string.
 * @param len          Length of character data to be encoded.
 * @return             Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int pe_ConstrainedStringData
(OSCTXT* pctxt, const char* string, const char* charSet,
 ...
 *                       - negative return value is error.
 */
#define pe_ConsUnsigned(pctxt, value, lower, upper) \
   pe_ConsUInt64(pctxt, value, lower, upper)
 ...
 *                       - negative return value is error.
 */
#define pe_ConsUnsignedSignedBound(pcxt, value, lower, upper) \
   pe_ConsUInt64SignedBound(pctxt, value, lower, upper)
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pe_ConsUInt64SignedBound (OSCTXT* pctxt, OSUINT64 value,
                             OSINT64 lower, OSINT64 upper);
...
/**
 * This function encodes a constrained whole number in the case of the value
 * range being greater than 64K as specified in Section 10.5 of the X.691
 * standard.
 *
 * @param pctxt            Pointer to context block structure.
 * @param adjusted_value   Unsigned adjusted integer value to be encoded. The
 *                            adjustment is done by subtracting the lower value
 *                            of the range from the value to be encoded.
 * @param len_bits         Number of bits in which to encode length value.
 * @return                 Completion status of operation:
 *                            - 0 (0) = success,
 *                            - negative return value is error.
 */
...
/**
 * This version of pe_GetMsgPtr return the length as a 32-bit unsigned
...
/**
 * This version of pe_GetMsgPtr return the length as a size-typed value
 * rather than a signed integer.
 *
 * @see pe_GetMsgPtr
 */
EXTERNPER OSOCTET* pe_GetMsgPtr64 (OSCTXT* pctxt, OSSIZE* pLength);
 ...
 *                       that must be maintained between function calls.
 * @param value        Length value to be encoded.
 * @return             If successful, then one of the following values:
 *                         - if value >= 16384, then fragmentation is required.
 *                            The return will be the number of values to encode
 *                            in the current fragment.  The return will be a
 *                            multiple of 16384, with the following constraints:
 *                               16384 <= return <= 65536
 *                               return <= value
 *                         - value (the given length)
 *                     Otherwise, a negative error status code.
 */
EXTERNPER int pe_Length (OSCTXT* pctxt, OSSIZE value);
...
/**
 * This function encodes a value of the ASN.1 object identifier type.
 * This version of the function allows the number of sub-identifiers and
 * sub-identifier array to be passed in separately thus making it possible
 * to use for both static and dynamic sub-identifier arrays.
 *
 * @param pctxt       Pointer to context block structure.
 * @param numids      Number of sub-identifers to encode.
 * @param pSubIds     Pointer to array holding sub-identifier values.
 * @return            Completion status of operation:
 *                       - 0 (0) = success,
 *                       - negative return value is error.
 */
EXTERNPER int perEncObjId
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pe_SmallLength (OSCTXT* pctxt, OSSIZE value);
 ...
 *                       - negative return value is error.
 */
#define pe_SemiConsInteger(pctxt, value, lower) \
   pe_SemiConsInt64(pctxt, value, lower)
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pe_SemiConsUnsignedSignedBound
   (OSCTXT* pctxt, OSUINT32 value, OSINT64 lower);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pe_SemiConsUInt64SignedBound
 ...
 *                       otherwise, a negative error status code.  Note that
 *                       in the non-error case, this will always be equal to
 *                       value unless a fragmented length is encoded.
 */
EXTERNPER int pe_UnconsLength (OSCTXT* pctxt, OSSIZE value);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pe_UnconsInt32 (OSCTXT* pctxt, OSINT32 value);

EXTERNPER int pe_UnconsInt32Aligned (OSCTXT* pctxt, OSINT32 value);
EXTERNPER int pe_UnconsInt32Unaligned (OSCTXT* pctxt, OSINT32 value);
 ...
 *                       - negative return value is error.
 */
EXTERNPER int pe_UnconsUInt32 (OSCTXT* pctxt, OSUINT32 value);

EXTERNPER int pe_UnconsUInt32Aligned (OSCTXT* pctxt, OSUINT32 value);
EXTERNPER int pe_UnconsUInt32Unaligned (OSCTXT* pctxt, OSUINT32 value);
...
EXTERNPER int pe_UnconsInt64Aligned (OSCTXT* pctxt, OSINT64 value);
EXTERNPER int pe_UnconsInt64Unaligned (OSCTXT* pctxt, OSINT64 value);
...
EXTERNPER int pe_UnconsUInt64Aligned (OSCTXT* pctxt, OSUINT64 value);
EXTERNPER int pe_UnconsUInt64Unaligned (OSCTXT* pctxt, OSUINT64 value);
#endif
 ...
 *                       to pu_newContext or rtInitContext and pu_setBuffer is
 *                       dumped.
 */
...
/**
 * This function resets the PER encode buffer to allow a new message
 * to be encoded into it.
 *
 * @param pctxt       Pointer to a context structure.
 * @return            Status of operation: 0 if successful, or a
 *                      negative value if an error occurs.
 */
EXTERNPER int pe_resetBuffer (OSCTXT* pctxt);
 ...
 *
 * @param pSizeList    A pointer to a size constraint structure.
 * @return             TRUE if size constraint is fixed.
 */
EXTERNPER OSBOOL pu_isFixedSize (const Asn1SizeCnst* pSizeList);

/**
 * This function is similar to the rtInitContext function in that it
 ...
 * rtInitContext to initialize it.
 *
...
/**
 * This function creates a new bit field in a diagnostics bit trace field
 * list.  It is now deprecated and has been replaced by the common
 * function rtxDiagNewBitField.
 *
 * @param pctxt       A pointer to a context structure.
 * @param nameSuffix  Suffix to be append to current element name.
 */
EXTERNPER PERField* pu_newField
(OSCTXT* pctxt, const char* nameSuffix);

/**
 * This function initializes a new bit field list for diagnostics
 * tracing.  It is used in code to encode or decode table-constrained
 * open type data to break down the patterns within the containers.
 *
 * @param pctxt       A pointer to a context structure.
 * @param bitOffset   Current bit offset.
 */
EXTERNPER void pu_initRtxDiagBitFieldList (OSCTXT* pctxt, OSINT16 bitOffset);

/**
 * Pop an element name from the diagnostics bit trace stack.  This
 * function is now deprecated and has been replaced with the common
 * function rtxCtxtPopElemName which is used maintain a name stack
 * for other purposes as well (for example, error reporting).
 *
 * @param pctxt       A pointer to a context structure.
 */
...
/**
 * Push an array element on the diagnostics bit trace stack.  This
 * function is now deprecated and has been replaced with the common
 * function rtxCtxtPushArrayElemName which is used maintain a name stack
 * for other purposes as well (for example, error reporting).
 *
 * @param pctxt       A pointer to a context structure.
...
/**
 * Push an element on the diagnostics bit trace stack.  This
 * function is now deprecated and has been replaced with the common
 * function rtxCtxtPushElemName which is used maintain a name stack
 * for other purposes as well (for example, error reporting).
 *
 * @param pctxt       A pointer to a context structure.
 ...
 *                       alphabet.
 * @return             Status value of zero if successful, or a negative
 *                       value if failure.
 */
EXTERNPER int pu_set16BitCharSet
(OSCTXT* pctxt, Asn116BitCharSet* pCharSet, Asn116BitCharSet* pAlphabet);

/**
 * This function sets a permitted alphabet character set for 16-bit character
 * strings from a sequential range of characters (for example, 'A'..'Z').
 *
 * @param pCharSet     Pointer to a character set structure describing the
 *                       character set currently associated with the character
 *                       string type. The resulting character set structure
...
EXTERNPER void pu_setRtxDiagOpenTypeFldList
...
EXTERNPER OSBOOL pu_BitAndOctetStringAlignmentTest
 ...
 *                       encode or decode buffer that was specified in the call
 *                       to rtInitContext or pu_newContext is dumped.
 ...
 *                       value if failure.
 */
EXTERNPER int pu_set32BitCharSet
 ...
 */
EXTERNPER int pu_GetLibVersion (OSVOIDARG);
 ...
 */
EXTERNPER const char* pu_GetLibInfo (OSVOIDARG);
