...
EXTRTMETHOD int rtCmpTCOID2(OSUINT32 keySize, const OSUINT32* keyIds,
                              OSUINT32 compSize, const OSUINT32* compIds)
{
   ...
   if (keySize == compSize)
   {
      ...
      for (i = 0; i < keySize; i++)
      {
         if (keyIds[i] != compIds[i])
         {
            return rtCmpTCUINT(&keyIds[i], &compIds[i]);
         }
      }
      /* All subIDS are equal */
      return 0;
   }
   return rtCmpTCUINT(&keySize, &compSize);
}
