...
/**
 * This macro logs an error in the time functions.  It checks to make sure
 * that the context parameter is not null before returning either a logged
 * error (stored in the context) or simply the status itself.
 *
 * @param   pctxt    A pointer to an OSCTXT data structure; it may be null.
 * @param   stat     The error status.
 *
 * @return  The result of calling LOG_RTERR or the status, depending on the
 *             value of the pctxt parameter.
 */
 ...
 public:
   ...
   */
   EXTRTMETHOD ASN1CTime
      (OSRTMessageBufferIF& msgBuf, char*& buf, OSSIZE bufSize,
       OSBOOL useDerRules);
   ...
   */
   EXTRTMETHOD ASN1CTime
      (OSRTMessageBufferIF& msgBuf, ASN1VisibleString& buf, OSBOOL useDerRules);
   ...
   */
   EXTRTMETHOD ASN1CTime
      (OSRTContext& ctxt, char*& buf, OSSIZE bufSize, OSBOOL useDerRules);
   ...
   *
   * Second's decimal fraction is represented by one or more digits from
   * 0 to 9. Note that the return value may differ for different inherited
   * ASN1CTime classes.
   *
   * @param             - none
   ...
   *
   * Second's decimal fraction is represented by one or more digits from
   * 0 to 9. Note that the action of this method may differ for different
   ...
   */
    ...
    *
    * @return  The length of the compiled time string.
    */
    ...
    *
    * @param   pbuf     The buffer to receive the time string.
    * @param   bufsize  The size of the buffer.
    *
    * @return  A character string containing the compiled time string.
    */
   EXTRTMETHOD const char* getTimeString (char* pbuf, OSSIZE bufsize);

   /**
    * This operator assigns this ASN1CTime object to the given ASN1CTime
    * reference.
    *
    * @return  A constant reference to the given ASN1CTime object.
    */
    ...
    *
    * @return  TRUE if this time is greater-than or equal to the given time.
    */
   virtual EXTRTMETHOD OSBOOL operator >= (ASN1CTime&);
    ...
    *
    * @return  TRUE if this time is less-than or equal to the given time.
    */
   virtual EXTRTMETHOD OSBOOL operator <= (ASN1CTime&);
...
typedef class _ASN1CTime : public ASN1CType {
 ...
 public:
   _ASN1CTime (OSRTMessageBufferIF& msgBuf, char*& , OSSIZE, OSBOOL) :
      ASN1CType (msgBuf) {}
   _ASN1CTime (OSRTMessageBufferIF& msgBuf, ASN1VisibleString&, OSBOOL) :
      ASN1CType (msgBuf) {}
   _ASN1CTime(OSRTContext& ctxt, char*& buf, OSSIZE bufSize,
...
/**
 * @}
 */ // end of ingroup asn1ctrl asn1ctime
