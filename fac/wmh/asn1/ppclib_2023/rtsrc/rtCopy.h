...
/**
 * The rtCopyOID function copies one ASN.1 OBJECT IDENTIFIER or RELATED-OID
 * value to another.
 *
 * The return value is one of the TRUE (copied successfully) or FALSE (error
 * has occurred).
...
/**
 * The rtCopyDynOID function copies one ASN.1 OBJECT IDENTIFIER or RELATED-OID
 * value to another.
 *
 * The return value is one of the TRUE (copied successfully) or FALSE (error
 * has occurred).
 *
 * @param srcOID       The pointer to the source dynamic object identifier
 *                       structure to copy.
 * @param dstOID       The pointer to destination dynamic OID structure to
 *                       receive the copied value. Memory is allocated for
 *                       the target sub-identifier array using rtxMemAlloc.
 * @return             The copying result. TRUE, if success, otherwise FALSE.
 */
EXTERNRT OSBOOL rtCopyDynOID
(OSCTXT* pctxt, const ASN1DynOBJID* srcOID, ASN1DynOBJID* dstOID);

/**
 * The rtCopyOIDtoDynOID function copies a static ASN.1 OBJECT IDENTIFIER
 * or RELATED-OID value to a dynamic OID structure.
 *
 * The return value is one of the TRUE (copied successfully) or FALSE (error
 * has occurred).
 *
 * @param srcOID       The pointer to the source static object identifier
 *                       structure to copy.
 * @param dstOID       The pointer to destination dynamic OID structure to
 *                       receive the copied value. Memory is allocated for
 *                       the target sub-identifier array using rtxMemAlloc.
 * @return             The copying result. TRUE, if success, otherwise FALSE.
 */
EXTERNRT OSBOOL rtCopyOIDtoDynOID
 ...
 * @return             The copying result. TRUE, if success, otherwise FALSE.
 */
EXTERNRT OSBOOL rtCopyOID64(const ASN1OID64* srcOID, ASN1OID64* dstOID);
