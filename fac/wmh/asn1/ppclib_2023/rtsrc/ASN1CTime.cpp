...
(OSRTMessageBufferIF& msgBuf, char*& buf, OSSIZE bufSize, OSBOOL useDerRules):
...
EXTRTMETHOD ASN1CTime::ASN1CTime(OSRTContext& ctxt, char*& buf, OSSIZE bufSize,
   OSBOOL useDerRules):
...
{
   OSSIZE len = OSCRTLSTRLEN(original.timeStr);
...
EXTRTMETHOD int ASN1CTime::getFractionStr(char* const pBuf, OSSIZE bufSize) {
   if(!parsed) {
      int stat = getTimeObj().parseString(timeStr);
      if(stat != 0)
         return LOG_TMERR(getCtxtPtr (), stat);
...
const ASN1CTime& ASN1CTime::operator= (const ASN1CTime& op) {
   ...
   parsed = op.parsed;
   checkCapacity ();
   rtxStrcpy (timeStr, strSize, op.timeStr);
   return *this;
}
...
EXTRTMETHOD const char* ASN1CTime::getTimeString (char* pbuf, OSSIZE bufsize)
{
   return rtxStrcpy (pbuf, bufsize, timeStr);
}
...
EXTRTMETHOD int ASN1CTime::equals (ASN1CTime& op)
{
   ...
   return getTimeObj().equals(op.getTimeObj());
}
