...
EXTRTMETHOD int ASN1TTime::getFractionStr
(char* const pBuf, OSSIZE bufSize) const
{
   if (mStatus != 0) return mStatus;
   if ((OSSIZE)mSecFracLen >= bufSize) return RTERR_STROVFLW;
...
EXTRTMETHOD int ASN1TTime::setFraction(int fraction, int fracLen) {
   mSecFraction = fraction;
   mSecFracLen = (fracLen < 0) ?
      int(log10 (double(fraction))) + 1 : fracLen;
   return 0;
}
...
{
   return (compileString(pbuf, bufsize) == 0) ? pbuf : 0;
}
...
EXTRTMETHOD char* ASN1TTime::toString (OSCTXT* pctxt) const
{
   ...
   if (compileString (tstr, MAX_TIMESTR_SIZE) == 0) {
      OSSIZE bufsize = OSCRTLSTRLEN(tstr) + 1;
      char* pStr = (char*)rtxMemAlloc (pctxt, bufsize);
      if (0 != pStr) {
         return rtxStrcpy (pStr, bufsize, tstr);
      }
   }
   return 0;
}
