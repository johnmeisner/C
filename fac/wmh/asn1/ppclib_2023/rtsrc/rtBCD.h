...
EXTERNRT const char* rtBCDToString
   (OSSIZE numocts, const OSOCTET* data, char* buffer, size_t bufsiz,
    OSBOOL isTBCD);
...
/**
 * This function converts a standard null-terminated string into a BCD
 * or TBCD value.  It is the 64-bit version of rtStringToDynBCD which
 * provides a clean interface to 64-bit size types.  However, the maximum
 * size of a string that can be converted is INT_MAX (the size a 32-bit
 * signed integer can hold).
 *
 * @param str          The source standard null-terminated string.
 * @param pctxt        Pointer to a context structure block.
 * @param poctstr      Pointer to a dynamic octet string variable.  Memory
 ...
 *                       a negative value if an error occurs.
 */
EXTERNRT int rtStringToDynBCD64
