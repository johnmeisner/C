...
/**
 * Encoding has bad alignment.  This occurs when an encoding is expected to
 * align with a byte boundary and does not.  It may happen with reference to
 * the start of the message or with reference to the start of a
 * length-constrained container.
 * Typically this means some bit string element has too few bits, where the
 * encoding of that element is supposed to end the encoding so that padding
 * cannot be used to acheive the desired alignment.
 */
#define ASN_E_BAD_ALIGN (ASN_E_BASE-12)

/**
 * Unknown PDU type.  This error occurs in interpreted BER decoders when
 * the PDU type for the message can't be automatically determined.  The
 * user in this case must manually specify the PDU type by some external
 * means.  For example, if the application has a command-line interface,
 * it may have a '-pdu' option for specifying the PDU type.
 */
#define ASN_E_UNKNOWNPDU (ASN_E_BASE-13)

/**
 * Encoded data does not conform to canonical rules for the current encoding
 * rules.
 */
...
/**
 * A value is not correctly fragmented as required by
 * the given encoding rules.
 */
