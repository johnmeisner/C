...
#include "rtAvn.h"
  ...
  * source files:
  *
  * AVNUNIDECFUNC (e.g., rtAvnReadUCS2)
  * AVNUNICHAR    (e.g., OSUNICHAR or OS32BITCHAR) */

EXTRTMETHOD int AVNUNIDECFUNC(OSCTXT* pctxt, AVNUNICHAR** ppCharStr,
   OSSIZE *pnchars)
{
   ...
   for (;;) {
      ...
      if (curCharOctet == 0) { /* first octet */
         ...
         else if (9 <= ub && ub <= 13)
         {
            ...
            AVNUNICHAR* start = (AVNUNICHAR*) OSMEMBUFPTR(&membuf);
            AVNUNICHAR* end = start + nchars;
            if (nchars > 0 && OS_ISSPACE(*end)) {
               /* There is some w/s to trim off. */
               OSSIZE cutchars = 1; /* w/s to cut */
               while (end > start && OS_ISSPACE(*(end-1))) {
                  end--;
                  cutchars++;
               }
               rtxMemBufCut(&membuf, (end - start) * sizeof(AVNUNICHAR),
                              cutchars * sizeof(AVNUNICHAR));
               nchars -= cutchars;
            }
         ...
         if (ub < 0x80) {
            curCharLen = 1;
            curUniChar = (AVNUNICHAR)(ub & 0x7F);
         }
         else if (ub < 0xE0) {
            curCharLen = 2;
            curUniChar = (AVNUNICHAR)(ub & 0x1F);
         }
         else if (ub < 0xF0) {
            curCharLen = 3;
            curUniChar = (AVNUNICHAR)(ub & 0xF);
         }
         else if (ub < 0xF8) {
            curCharLen = 4;
            curUniChar = (AVNUNICHAR)(ub & 0x7);
         }
         else if (ub < 0xFC) {
            curCharLen = 5;
            curUniChar = (AVNUNICHAR)(ub & 0x3);
         }
         else if (ub < 0xFE) {
            curCharLen = 6;
            curUniChar = (AVNUNICHAR)(ub & 0x1);
         }
         ...
         if ((curCharLen > 3) && (sizeof(AVNUNICHAR) < 4)) {
         ...
         curUniChar = (AVNUNICHAR)((curUniChar << 6) | (ub & 0x3F));
      }
      else {
   ...
   *ppCharStr = rtxMemAlloc(pctxt, nchars * sizeof(AVNUNICHAR));
   OSCRTLSAFEMEMCPY(*ppCharStr, nchars * sizeof(AVNUNICHAR), membuf.buffer,
      nchars * sizeof(AVNUNICHAR));
   *pnchars = nchars;
