...
EXTERNRT OSBOOL rtCmpUInt64 (const char* name, OSUINT64 value,
   ...
   #define rtCmpSize(name, value, compValue, errBuff, errBuffSize) \
      rtCmpUnsigned(name, value, compValue, errBuff, errBuffSize)
#elif defined OSSIZE_IS_64BITS
   #define rtCmpSize(name, value, compValue, errBuff, errBuffSize) \
      rtCmpUInt64(name, value, compValue, errBuff, errBuffSize)
#endif
 ...
 * @param numbits      The number of bits in the first value to compare.
 * @param data         The pointer to the data of the first value to compare.
 * @param dataSize     Size of the array pointed to by data and compData.
...
/**
 * The rtCmpOIDValue function compares two ASN.1 OBJECT IDENTIFIER or
 * REALTIVE-OID values.
 *
 * @param name         The name of value. Used for constructing errBuff if
 *                       values are not matching.
...
/**
 * This function compares an array of subidentifiers of two OBJECT IDENTIFIER
 * or RELATIVE-OID values for equality.
 *
 * @param name         The name of value. Used for constructing errBuff if
 *                       values are not matching.
 * @param numids1      Number of subidentifiers in first OID value.
 * @param numids2      Number of subidentifiers in second OID value.
 * @param subids1      Pointer to array of subidentifiers in first OID value.
 * @param subids2      Pointer to array of subidentifiers in second OID value.
 * @param errBuff      The pointer to the buffer to receive the null-terminated
 *                       string if the comparison fails. If the comparison
 *                       failed, this buffer will contain the null-terminated
 *                       string that explains the reason of comparison failure.
 ...
 *                       otherwise FALSE.
 */
EXTERNRT OSBOOL rtCmpOID2
(const char* name, OSUINT32 numids1, OSUINT32 numids2,
 const OSUINT32* subids1, const OSUINT32* subids2,
 char* errBuff, OSSIZE errBuffSize);

/**
 * This function prints information on two OBJECT IDENTIFIER or
 * RELATIVE-OID values that are not equal.
 *
 * @param name         The name of value. Used for constructing errBuff if
 *                       values are not matching.
 * @param numids1      Number of subidentifiers in first OID value.
 ...
 *                       otherwise FALSE.
 */
EXTERNRT void rtCmpPrtToStrOIDsNotEqual
