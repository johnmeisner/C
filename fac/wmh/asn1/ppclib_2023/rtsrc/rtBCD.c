...
(const char* str, OSOCTET* bcdStr, OSSIZE bufsiz, OSBOOL isTBCD)
{
   ...
   /*
   Previous code (prior to v7.7) allowed A through F.  But since F is the
   filler character when the number of digits is odd, it seems like we should
   only allow A through E.  That's what the Java run-time allows.
   */
   for (i = 0; *pstr != 0 && i < maxDigits; pstr++, i++) {
      sym = (OSOCTET) OS_TOUPPER (*pstr);
      isDigit = OS_ISDIGIT(sym);
      if (!isDigit && (sym < 'A' || sym > 'E'))
         break;
...
EXTRTMETHOD int rtStringToDynBCD64
(OSCTXT* pctxt, const char* str, OSDynOctStr64* poctstr, OSBOOL isTBCD)
{
   int len;
   OSOCTET* pdata;

   if (0 == poctstr) return RTERR_INVPARAM;
   poctstr->numocts = (OSCRTLSTRLEN(str)/2) + 1;

   pdata = (OSOCTET*) rtxMemAlloc (pctxt, poctstr->numocts);
   if (0 == pdata) return RTERR_NOMEM;

   len = rtStringToBCD (str, pdata, poctstr->numocts, isTBCD);
   if (len >= 0) poctstr->numocts = (OSSIZE)len;
...
{
   ASN1DynOctStr64 dynOS64;
   int len = rtStringToDynBCD64(pctxt, str, &dynOS64, FALSE);
   if (len >= 0) {
      poctstr->numocts = (OSUINT32)len;
      poctstr->data = dynOS64.data;
   }
   return len;
}

EXTRTMETHOD const char* rtBCDToString (OSSIZE numocts,
   const OSOCTET* data, char* buffer, OSSIZE bufsiz, OSBOOL isTBCD)
{
   if (isTBCD) {
      ...
      /* Check for invalid args or overflow */
      if (!data || !buffer || !numocts || !bufsiz || maxDigits < numocts)
         return NULL;
...
EXTRTMETHOD const char* rtTBCDToString (OSSIZE numocts, const OSOCTET* data,
