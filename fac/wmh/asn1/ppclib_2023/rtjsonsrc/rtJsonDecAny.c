...
/* Match JSON object, array, or value. (Previously, matched only object or
   array.)
@param pmembuf Buffer to receive decoded JSON value.  If null, the value is
               not captured and UTF-16 encodings are not checked for
               validity (meaning, we don't ensure 16-bit high/low surrogate
               characters come in pairs as they ought to).
*/
static int jsonDecAnyType (OSCTXT* pctxt, OSRTMEMBUF* pmembuf)
{
   ...
   for (;;) {
      ...
      RTDIAG2 (pctxt, "read character: %c\n", pctxt->lastChar);
      ...
      if (next_state >= 0) {
         ...
         if (MATCHING_NUMBER(pctxt) && next_state == OK && mode == MODE_DONE)
         {
            ...
            if ( pmembuf ) rtxMemBufCut(pmembuf,
                                       rtxMemBufGetDataLen(pmembuf)-1, 1);
         }
         ...
         case UC: {
            ...
            /* else: utf16hs will be 0; state will be set to ST */
         ...
         case -7:
            ...
            if (MATCHING_NUMBER(pctxt) && mode == MODE_DONE) {
               ...
               stat = popMode(&parseStack, MODE_ARRAY);
               if (0 != stat) return LOG_RTERR(pctxt, RTERR_UNBAL);
            }
