...
/**
 * @param ppvalue If dynamic, this is an output parameter and it receives the
 *                pointer to newly allocated string. If dynamic, this is
 *                an input parameter and is the buffer to decode into.
 * @param bufsize If not dynamic, the size of the buffer; otherwise ignored.
 * @param dynamic
 */
int rtJsonDecStringValueInternal (OSCTXT* pctxt, OSUTF8CHAR** ppvalue,
                                    OSSIZE bufsize, OSBOOL dynamic)
{
   OSRTMEMBUF membuf;
   int stat;
   OSOCTET ub;
   ...
   if ( dynamic ) rtxMemBufInit (pctxt, &membuf, 128);
   else rtxMemBufInitBuffer(pctxt, &membuf, *ppvalue, bufsize, 0);
   ...
   if (dynamic) *ppvalue = membuf.buffer;
...
int rtJsonDecStringValue(OSCTXT* pctxt, OSUTF8CHAR** ppvalue)
{
   ...
   ret = rtJsonDecStringValueInternal(pctxt, &pvalue, 0, TRUE);
   *ppvalue = pvalue;
...
{
   return rtJsonDecStringValueInternal(pctxt, &pvalue, bufsize, FALSE);
}
