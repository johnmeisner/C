...
typedef struct OSXMLReader {
   ...
   /* mbNoTransform: if true, do not convert character references and entities.
   */
   ...
   /* OSXMLSI_* values are used as indices into mLocalStates.  This is used to
      save the transition state for the various process* functions.
      For example, processCharacters stores its state in
      mLocalStates[OSXMLSI_CONTENT]
   */
