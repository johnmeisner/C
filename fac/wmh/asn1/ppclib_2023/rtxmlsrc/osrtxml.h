...
/*! \mainpage C XML Runtime Library Functions
 *
...
#define OSHASDEFAULT    0x00000010  /* decode should accept values which
				       ...
				       value				*/
#define OSASN1XER       0x00000008  /* if true, -xml code and runtime
...
/**
 * OSXMLGroupDesc describes how entries in an OSXMLElemIDRec array make up a
 * group. Here, "group" means a set of elements, any of which may be matched
 * next. This does not correspond directly to an XSD group.
 *
 * For example, if elementA is optional and followed by non-optional elementB,
 * then there will be a group that contains both elements.  There will also be a
 * group that contains only elementB; this will be the group of interest after
 * elementA is matched.
 */
typedef struct OSXMLGroupDesc {
   /* row: identifies the index in an OSXMLElemIDRec[] for the first element
      that is in this group.
   */
   int row;

   /* num: The number of elements that are in this group, each of which has
      a row in the OSXMLElemIDRec array.  Thus, if tab is such an array, the
      elements for this group are found in tab[row]..tab[row+num-1] */
   int num;

   /* anyCase: The id to return when an unexpected element is found when
      matching against the group. -1 if an unexpected element should not
      be matched.
   */
   int anyCase;
} OSXMLGroupDesc;
...
EXTERNXML int rtXmlGetBase64StrDecodedLen
   (const OSUTF8CHAR* inpdata, OSSIZE srcDataSize,
    OSSIZE* pNumOcts, OSSIZE* pSrcDataLen);
...
/**
 * This function is identical to rtXmlDecBase64Str except that is supports
...
EXTERNXML int rtXmlDecBase64StrValue (OSCTXT* pctxt,
                                      OSOCTET* pvalue, OSUINT32* pnocts,
                                      OSSIZE bufSize, OSSIZE srcDataLen);


/**
 * This function decodes is identical to rtXmlDecBase64StrValue except that
 * it supports a 64-bit integer length on 64-bit systems.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
...
/**
 * This function is identical to rtXmlDecDynBase64Str except that it supports
 * a 64-bit integer length on 64-bit systems.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
...
/**
 * This function is identical to rtXmlDecDynHexStr except that it supports
 * a 64-bit integer length on 64-bit systems.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
...
(OSCTXT* pctxt, OSUTF8CHAR* outdata, OSSIZE max_len);
...
/**
 * This function is identical to rtXmlDecHexStr except that it supports a 64-bit
 * integer length on 64-bit systems.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
...
EXTERNXML int rtXmlDecHexStrValue (OSCTXT* pctxt,
   const OSUTF8CHAR* const inpdata, OSSIZE nbytes, OSOCTET* pvalue,
   OSUINT32* pnbits, OSINT32 bufsize);

EXTERNXML int rtXmlDecHexStrValue64 (OSCTXT* pctxt,
   const OSUTF8CHAR* const inpdata, OSSIZE nbytes, OSOCTET* pvalue,
 ...
 *                       - negative return value is error.
 */
EXTERNXML int rtXmlEncBase64Binary (OSCTXT* pctxt, OSSIZE nocts,
                                    const OSOCTET* value,
                                    const OSUTF8CHAR* elemName,
                                    OSXMLNamespace* pNS);
...
EXTERNXML int rtXmlEncBase64BinaryAttr
   (OSCTXT* pctxt, OSUINT32 nocts, const OSOCTET* value,
    const OSUTF8CHAR* attrName, OSSIZE attrNameLen);
 ...
 *                       - negative return value is error.
 */
EXTERNXML int rtXmlEncBase64StrValue (OSCTXT* pctxt, OSSIZE nocts,
                                      const OSOCTET* value);
 ...
 *                       - negative return value is error.
 */
EXTERNXML int rtXmlEncBitString (OSCTXT* pctxt, OSSIZE nbits,
                                 const OSOCTET* value,
                                 const OSUTF8CHAR* elemName,
                                 OSXMLNamespace* pNS);
...
/**
 * This function encodes a variable of the ASN.1 BIT STRING type.
 * The encoded data is a sequence of '1's and '0's.  This is used
 * for BIT STRINGs with extdata member present.
 *
 * @param pctxt        Pointer to context block structure.
 * @param nbits        Number of bits in the bit string.
 * @param value        Value to be encoded.
 * @param dataSize     Size of data member.
 * @param extValue     Value of extdata to be encoded.
 * @param elemName     XML element name.  A name must be provided.
 *                       If an empty string is passed (""), no element tag is
 *                       added to the encoded value.
 * @param pNS          Pointer to namespace structure.
 ...
 *                       - negative return value is error.
 */
EXTERNXML int rtXmlEncBitStringExt (OSCTXT* pctxt, OSSIZE nbits,
                                    const OSOCTET* value,
                                    OSSIZE dataSize,
...
/**
 * Sort (as for CXER, Canonical-XER) and encode previously encoded SET OF
 * components.
 * @param pctxt      Context to use to encode the sorted encoding
 * @param pBufCtxt   Context previously used to encode the components which
 *                   are to be sorted.
 * @param pList      List of OSRTBufLocDescr identifying the location of each
 *                   of the components' encodings within pBufCtxt.
 */
EXTERNXML int rtXmlEncCanonicalSort(OSCTXT* pctxt, OSCTXT* pBufCtxt,
                                    OSRTSList* pList);
 ...
 *                       - negative return value is error.
 */
EXTERNXML int rtXmlEncHexBinary (OSCTXT* pctxt, OSSIZE nocts,
                                 const OSOCTET* value,
                                 const OSUTF8CHAR* elemName,
                                 OSXMLNamespace* pNS);
...
(OSCTXT* pctxt, const OSUTF8CHAR* name, OSSIZE nameLen,
 const OSUTF8CHAR* value, OSSIZE valueLen, OSSIZE itemCount);
...
EXTERNXML int rtXmlEncStringValue2
   (OSCTXT* pctxt, const OSUTF8CHAR* value, OSSIZE valueLen);
 ...
 *
 * This function should not be called with a context that has an associated
 * stream open, but if it is, the stream may be automatically closed.
 *
...
(OSCTXT* pctxt, OSSIZE minLength, OSSIZE maxLength);
...
EXTERNXML int rtSaxTestAttributesPresent (OSCTXT* pctxt,
   ...
   const OSUTF8CHAR* const* attrNames, OSSIZE numOfAttrs,
...
(OSCTXT* pctxt, const OSUTF8CHAR* value, OSSIZE len);
...
/**
 * This version of the rtXmlpDecAny function returns the string in
 * a mutable buffer.
 *
 * @param pctxt        Pointer to context block structure.
 * @param pvalue       Pointer to UTF8 character string pointer to
...
(OSCTXT* pctxt, const OSUTF8CHAR** ppAttrStr, OSSIZE attrIndex);
...
/**
 * This function is identical to rtXmlpDecBase64Str except that it supports
...
/**
 * This function is identical to rtXmlpDecBitString except that it supports
 ...
 * the user provides a pre-allocated memory buffer to receive the decoded
 * data.  One byte in a memory buffer can hold 8 characters of encoded
 * data.  Bits are stored from MSB to LSB order.  This version supports
 * BIT STRINGs with the extdata member present.
 *
 * @param pctxt        Pointer to context block structure.
 * @param pvalue       Pointer to a variable to receive the decoded boolean
 *                       value.
 * @param pnbits       Pointer to hold decoded number of bits.
 * @param ppextdata    Pointer to byte array containing extension data.
 * @param bufsize      Size of buffer passed in pvalue argument.
 * @return             Completion status of operation:
 *                       - 0 = success,
 *                       - negative return value is error.
 */
EXTERNXML int rtXmlpDecBitStringExt(OSCTXT* pctxt, OSOCTET* pvalue,
      OSUINT32* pnbits, OSOCTET** ppextdata, OSUINT32 bufsize);

/**
 * This function is identical to rtXmlpDecBitStringExt except that it supports
 * lengths up to 64-bits in size on 64-bit machines.
 *
 * This function decodes a bit string value.  The string consists of a
 * series of '1' and '0' characters.  This is the static version in which
 ...
 *                       - 0 = success,
 *                       - negative return value is error.
 * @see rtXmlpDecBitStringExt
 */
EXTERNXML int rtXmlpDecBitStringExt64(OSCTXT* pctxt, OSOCTET* pvalue,
      OSSIZE* pnbits, OSOCTET** ppextdata, OSSIZE bufsize);
...
/**
 * This function is identical to rtXmlpDecDynBase64Str except that it supports
 * 64-bit integer lengths on 64-bit systems.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
...
/**
 * This function is identical to the rtXmlpDecDynHexStr except that it
 * supports lengths up to 64 bits in size on 64-bit systems.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
...
(OSCTXT* pctxt, OSUTF8CHAR* out, OSSIZE max_len);
...
/**
 * This function is identical to rtXmlpDecHexStr except that it supports
 * lengths up to 64-bits in size on 64-bit machines.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
...
EXTERNXML int rtXmlpDecUInt64 (OSCTXT* pctxt, OSUINT64* pvalue);
   ...
   #define rtXmlpDecSize(pctxt, pvalue) rtXmlpDecUInt(pctxt, pvalue)
#elif defined OSSIZE_IS_64BITS
   #define rtXmlpDecSize(pctxt, pvalue) rtXmlpDecUInt64(pctxt, pvalue)
#endif
...
/**
 * This function decodes XSI (XML Schema Instance) attributes that may be
 * present in any arbitrary XML element within a document.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
 ...
 * attribute (xsi:type).
 *
 * Prior to calling this function, use rtXmlpSelectAttribute to select the
 * attribute.  After calling this function, if you want to read the same
 * attribute again, you will need to call rtXmlpSelectAttribute again.
 *
 * @param pctxt        A pointer to a context structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
...
(const OSXMLStrFragment* attrName, OSINT16 nsidx,
 OSSIZE nAttr, const OSXMLAttrDescr attrNames[], OSUINT32 attrPresent[]);
 ...
 *
 * If this reaches the closing tag for the current level, it returns XML_OK_EOB.
 * If this encounters an unexpected element and !continueParse, it returns
 * RTERR_UNEXPELEM (without logging it).
 *
 * @param pctxt        Pointer to context block structure.
 ...
 * @param continueParse When value equals TRUE function skips unrecognized
 *                        elements; skipped elements are logged, but an error is
 *                        not returned.
 * @return             Completion status of operation:
...
(OSCTXT* pctxt, const OSXMLElemIDRec* tab, OSSIZE nrows, OSINT32 level,
 ...
 * @param attrIndex    Index of selected attribute.
 * @return             Completion status of operation:
 *                       - positive or zero return value is
 *                          attribute index in descriptor table,
 *                       - negative return value is error.
...
(OSCTXT* pctxt, OSXMLNameFragments* pAttr,  OSINT16* nsidx, OSSIZE attrIndex);
 ...
 * @param groups       Number of groups in groups table. If checkRepeat is
 *                     FALSE, you can pass 0 for this if the value is unknown.
 ...
 * @param checkRepeat  If true, this method is being called to check for a
 *                     repeat of curID. In this case, we do not treat curID as
 *                     being required.  Otherwise, curID is required if
 *                     curId <= lastMandatoryID.
 * @return             Completion status of operation:
 *                       - positive or zero value is element identifier,
 *                       - negative return value is error.
 */
 ...
 *    - If the current decode group includes an any case, unexpected elements
 *      will be matched against it (the any case id will be returned).
 *    - Otherwise, if groupMode is true, and the element identified by
 *      lastMandatoryID has been reached, XML_OK_EOB is returned.  The
 *      unexpected element may belong to something following the elements in
 *      tab.
 *    - If neither of the above applies, unknown elements will be logged and
 *      skipped over, with this method continuing as if the unexpected element
 *      were not present.
 * Handling of the case of reaching closing tag for current level:
 *    - If the last mandatory element is reached, return XML_OK_EOB.
 *    - Otherwise, log and return XML_E_ELEMSMISRQ.
 *
 * This function is equivalent to:
 *    rtXmlpGetNextSeqElemID2(pctxt, tab, pGroup, curID, lastMandatoryID,
 *                            groupMode, FALSE);
 *
 *
 * @param pctxt        Pointer to context block structure.
 ...
 * @param curID        Current decode group.
 * @param lastMandatoryID Identifier of last mandatory element.
 * @param groupMode    This parameter must be setted to TRUE when decoding
 *                      groups or base types.
 * @return             Completion status of operation:
 *                       - positive or zero value is element identifier,
 *                       - negative return value is error.
...
/**
 * This is an ASN.1 extension-supporting version of rtXmlpGetNextSeqElemID.  It
 * allows required extension elements to be absent, except that when an
 * extension group is present, all required extension elements in that group
 * must be present.  It otherwise behaves the same as rtXmlpGetNextSeqElemID.
 *
 * @param pctxt        Pointer to context block structure.
 * @param tab          Elements descriptor table.
 * @param pGroup       Decode groups table.
 * @param extRequired  extRequired[i] corresponds to pGroup[i].  This is TRUE
 *       if and only if the decode group ends with a non-optional, non-default
 *       extension element that must be present in the encoding (because it is
 *       inside an extension group for which some element has already been
 *       decoded).
 * @param postExtRootID This is the group id corresponding to the decode group
 *       that begins with the first root element that follows the extension
 *       additions.  If there is no such element, this is -1.
 * @param curID        Current decode group.
 * @param lastMandatoryID Identifier of last mandatory element.
 * @param groupMode    This parameter must be setted to TRUE when decoding
 *                      groups or base types.
...
(OSCTXT* pctxt, const OSXMLElemIDRec* tab, OSSIZE nrows,
 const OSUINT8* pOrder, OSSIZE nOrder, OSSIZE maxOrder, int anyID);
...
(OSCTXT* pctxt, const OSXMLElemIDRec* tab, OSSIZE nrows,
 const OSUINT16* pOrder, OSSIZE nOrder, OSSIZE maxOrder, int anyID);
...
(OSCTXT* pctxt, const OSXMLElemIDRec* tab, OSSIZE nrows,
 const OSUINT32* pOrder, OSSIZE nOrder, OSSIZE maxOrder, int anyID);
...
(OSCTXT* pctxt, const OSUTF8CHAR* namespaceTable[], OSSIZE nmNamespaces);
...
(OSCTXT* pctxt, const OSXMLItemDescr typetab[], OSSIZE typetabsiz);
