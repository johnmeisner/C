...
int xd_objid
(OSCTXT *pctxt, ASN1OBJID *object_p, ASN1TagType tagging, int length)
{
   register int	status;
   ...
   /* Decode OID sub-identifiers */

   status = rtDecOIDSubIds(pctxt, object_p->subid, ASN_K_MAXSUBIDS, length);
   if (status < 0) return LOG_RTERR(pctxt, status);
