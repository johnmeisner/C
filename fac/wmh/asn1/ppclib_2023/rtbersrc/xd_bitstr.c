                 ...
                 ASN1TagType tagging, OSSIZE length, OSBOOL indefLen)
{
   ...
   *object_p2 = pbitstr;
   ...
   if (rtxCtxtTestFlag(pctxt, ASN1DER) || rtxCtxtTestFlag(pctxt, ASN1CANON))
   {
      int lastBits = (int) (*numbits_p % 8);
      if ( lastBits != 0 ) {
         int unusedBits = 8 - lastBits;
         int mask = (1 << unusedBits) - 1;   /* lowest unusedBits set */
         if (((*object_p2)[*numbits_p / 8] & mask) != 0)
         {
            /* Some unused bit is not zero. Use *numbits_p and not ll to locate
            the last byte in the bit string, since ll can include any number of
            initial octets that are not part of the bit string, due to
            fragmentation. */
            rtxErrAddStrParm(pctxt, "unused bits must be zeroed");
