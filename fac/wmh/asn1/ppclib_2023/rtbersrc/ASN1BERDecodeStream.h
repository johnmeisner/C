...
class EXTERNBER ASN1BERDecodeStream : public ASN1MessageBuffer
{
 ...
 public:
   ...
   /**
    * This method decodes a variable of the ASN.1 OCTET STRING type. It will
    * allocate dynamic memory to store the decoded result.  It supports
    * 64-bit length variables.
    *
    * @param val                       Reference to an ASN1DynOctStr variable
    *                                    to receive the decoded octet string.
    * @param tagging                   An enumerated type whose value is set to
    ...
    *                                    If explicit, the length is obtained
    *                                    from the decoded length field.
    * @param indefLen                  Flag indicating if component to be
    *                                    decoded is indefinite length.
    *                                    This parameter only has meaning if
    *                                    the tagging parameter specifies
    *                                    implicit decoding.
    *                                    If explicit, the length is obtained
    *                                    from the decoded length field.
    * @return                          Completion status of operation:
    *                                    - 0 (0) = success,
    *                                    - negative return value is error.
    * @see                            ::berDecStrmDynOctStr64
    */
   int decodeOctStr
      (OSDynOctStr64& val, ASN1TagType tagging = ASN1EXPL,
       OSSIZE length = 0, OSBOOL indefLen = FALSE);
   ...
   /**
    * This method decodes the tag and length at the current decode pointer
    * location and returns the results.  This variant of the method
    * supports 64-bit length values.
    *
    * @param tag                       Reference to a variable to receive the
    *                                    decoded ASN.1 tag value.
    * @param len                       Reference to an object to receive the
    *                                    decoded length of the tagged
    *                                    component.
    * @return                          Completion status of operation:
    *                                    - 0 (0) = success,
