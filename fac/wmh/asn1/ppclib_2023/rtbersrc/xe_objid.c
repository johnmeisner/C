...
(OSCTXT* pctxt, OSSIZE numids, const OSUINT32* pSubIds, ASN1TagType tagging)
{
   ...
   for (aal = 0, i = numids - 1; i >= 1 && aal >= 0; i--)
   {
      subid = (i == 1) ?
	 (pSubIds[0] * 40) + pSubIds[1] : pSubIds[i];
...
int xe_objid (OSCTXT* pctxt, ASN1OBJID *object_p, ASN1TagType tagging)
{
   return (0 == object_p) ? LOG_RTERR (pctxt, ASN_E_INVOBJID) :
      encodeOID(pctxt, object_p->numids, object_p->subid, tagging);
}

int xe_dynObjId (OSCTXT* pctxt, ASN1DynOBJID *object_p, ASN1TagType tagging)
{
   return (0 == object_p || 0 == object_p->pSubIds) ?
      LOG_RTERR (pctxt, ASN_E_INVOBJID) :
      encodeOID(pctxt, object_p->numids, object_p->pSubIds, tagging);
}
