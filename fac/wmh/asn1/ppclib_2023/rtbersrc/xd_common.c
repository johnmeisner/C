...
(OSCTXT *pctxt, ASN1TAG *tag_p, OSSIZE *len_p, OSBOOL* pIndefLen, OSOCTET flags)
{
   ...
   OSRTBUFSAVE (pctxt);
   errcnt = pctxt->errInfo.list.count;
   ...
   if (!(flags & XM_ADVANCE)) {
      ...
      if ( stat == 0 )
      {
         /* We might have logged some non-fatal errors.  If so, remove them. */
         OSSIZE removeCnt = pctxt->errInfo.list.count - errcnt;
         if (removeCnt > 0 ) rtxErrResetLastErrors(pctxt, removeCnt);
      }
   }
