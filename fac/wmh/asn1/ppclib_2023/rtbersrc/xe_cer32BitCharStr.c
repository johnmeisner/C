...
int xe_cer32BitCharStr (OSCTXT* pctxt, Asn132BitCharString* object_p,
                     ASN1TagType tagging, ASN1TAG tag)
{
   /* At most 250 characters/1000 octets per fragment. */
   OSOCTET lbuf[4];
   ...
   indeflen = object_p->nchars > 250;
   fragments = (object_p->nchars + 250) / 250;
   fragSize = object_p->nchars - 250 * (fragments - 1);
   ...
   if (object_p->nchars > 0)
   {
      ...
      while (fragments > 0)
      {
         ...
         for (;;)
         {
            ...
            OS32BITCHAR sym = object_p->data[idx];
