 ...
 * @return             Completion status of operation: 0 (0) = success with no
 *                      fatal errors, negative return value is a fatal error.
 */
 ...
 *
 * BER requires that INTEGER be encoded in a minimal number of octets. This
 * function does not enforce that rule because it may be used to decode integer
 * values that are not ASN.1 INTEGER values. However, if the ASN1CANON or
 * ASN1DER flags are set, this will log a non-fatal error if the minimal
 * number of octets is not used.
 *
 * @param pctxt       Pointer to context block structure.
 * @param tagging      Specifies whether element is implicitly or explicitly
 *                       tagged.
...
#define xd_size(pctxt, object_p, tagging, length) \
      xd_unsigned(pctxt, object_p, tagging, length)
#elif defined OSSIZE_IS_64BITS
#define xd_size(pctxt, object_p, tagging, length) \
      xd_uint64(pctxt, object_p, tagging, length)
#endif
...
/**
 * This function decodes a value of the ASN.1 object identifier type
 * into a structure containing a dynamic array of subidentifiers.
 *
 * @param pctxt       Pointer to context block structure.
 * @param object_p     Pointer to value to receive decoded result. The
 *                       ASN1OBJID structure contains an integer to hold the
 *                       number of subidentifiers and a dynamic array to hold
 *                       the subidentifier values. Memory for this array is
 *                       allocated using rtxMemAlloc and must be freed using
 *                       rtxMemFreePtr.
 * @param tagging      Specifies whether element is implicitly or explicitly
 *                       tagged.
 * @param length       Length of data to retrieve. Valid for implicit case
...
/**
 * This function decodes an ASN.1 (unconstrained) REAL value.
 * This function allows the REAL to be encoded in any base, but if the
 * ASN1CANON or ASN1DER flags are set, it will log a non-fatal error if the
 * CER/DER restrictions are not met.
 *
 * @param pctxt       Pointer to context block structure.
...
/**
 * This function decodes an ASN.1 REAL value that is a base 2 value, -0, NaN,
 * or +/-INF (base 10 values are not allowed).
 *
 * This function allows a base 2 value to be encoded in bases 2, 8, or 16 (not
 * base 10), but if the ASN1CANON or ASN1DER flags are set, it will log a
 * non-fatal error if the encoding uses base 8 or 16 or if some other CER/DER
...
/**
* This function decodes the content octets of an ASN.1 REAL base 10 encoding.
* This function is for internal use by the runtime.
*
* If the ASN1CANON or ASN1DER flags are set, this will log a non-fatal error if
* the CER/DER restrictions are not met.  This includes enforcing the use
* of the NR3 form.
*
* @param pctxt       Pointer to context block structure.
* @param object_p     Pointer to value to receive decoded result. The OSREAL
*                       data type is a double-precision floating point number
*                       type (C double type).
* @param content     The content octets, excluding the first octet which
*                    encodes the base and form.
* @param length       Length of data pointed to by content.
* @param form        The NR form used (1, 2, or 3).  This governs the set of
*                    valid characters and will be check for validity if the
*                    ASN1CANON or ASN1DER flag is set.
* @return             Completion status of operation:
*                       - 0 (0) = success,
*                       - negative return value is error.
*/
EXTERNBER int xd_real_b10_content(OSCTXT *pctxt, OSREAL *object_p,
   const char* content, int length, int form);
...
/**
 * This function decodes an ASN.1 base 10 REAL value (i.e. the type is
 * constrained to base 10 values).
 *
 * This function will log an error if the value is not encoded in base 10, is
 * -0, NaN, or +/-INF.
 *
 * @param pctxt       Pointer to context block structure.
 * @param object_p     Pointer to a character pointer variable to receive the
...
#if defined OSSIZE_IS_32BITS
   #define xe_size(pctxt,object_p,tagging) \
      xe_unsigned(pctxt, object_p, tagging)
#elif defined OSSIZE_IS_64BITS
   #define xe_size(pctxt,object_p,tagging) \
      xe_uint64(pctxt, object_p, tagging)
#endif
...
/**
 * This function encodes an ASN.1 BIT STRING according to CER, except that it
 * assumes any trailing zero bits that must be trimmed have already been
 * trimmed.
 *
 * A BIT STRING of 7992 bits or less is encoded in primitive form.  All other
 * values are encoded in constructed form.  When encoding in constructed form,
 * if tagging is ASN1IMPL, the caller is responsible for encoding the EOC (prior
 * to calling this function), the tag (which must use constructed form), and the
 * length (which must be indefinite); if tagging is ASN1EXPL (and constructed
 * form applies), the full TLV-EOC is encoded.
 *
 * @param pctxt      Pointer to ASN.1 context block structure
 * @param object_p   Bit string to be encoded.
 * @param numbits	   Number of bits in the bit string.
 * @param tagging    Explicit or implicit tagging specification.
 * @returns          Length of the encoded message component, or negative if
 *                   encoding fails.
 */
EXTERNBER int xe_cerBitstr(OSCTXT* pctxt, const OSOCTET* object_p,
                           OSSIZE numbits,
                           ASN1TagType tagging);
...
*
* This assumes each C char corresponds to an octet in the encoding.
* An character string of 1000 characters or less is encoded in primitive form.
* This takes into account the ASN1ESCZTERM flag and "\0" escape sequence.  All
* other values are encoded in constructed form.  When encoding in constructed
* form, if tagging is ASN1IMPL, the caller is responsible for encoding the EOC
* (prior to calling this function), the tag (which must use constructed form),
* and the length (which must be indefinite); if tagging is ASN1EXPL (and
* constructed form applies), the full TLV-EOC is encoded.
*
* @param pctxt      Pointer to ASN.1 context block structure
* @param pvalue     null-terminated C string
* @param tagging    Explicit or implicit tagging specification.
...
*                   encoding fails.
*/
EXTERNBER int xe_cerCharstr(OSCTXT* pctxt, const char* pvalue,
                              ASN1TagType tagging, ASN1TAG tag);
...
*
* An OCTET STRING of 1000 octets or less is encoded in primitive form.  All
* other values are encoded in constructed form.  When encoding in constructed
* form, if tagging is ASN1IMPL, the caller is responsible for encoding the EOC
* (prior to calling this function), the tag (which must use constructed form),
* and the length (which must be indefinite); if tagging is ASN1EXPL (and
...
*
* @param pctxt      Pointer to ASN.1 context block structure
* @param object_p   octet string to be encoded.
* @param numocts    Number of octets in the octet string.
* @param tagging    Explicit or implicit tagging specification.
* @returns          Length of the encoded message component, or negative if
*                   encoding fails.
*/
EXTERNBER int xe_cerOctstr(OSCTXT* pctxt, const OSOCTET* object_p,
                           OSSIZE numocts,
                           ASN1TagType tagging);
...
xe_charstr (pctxt, (const char*)object_p, tagging, ASN_ID_UTF8String)
 ...
 /**
 * This function will encode an ASN.1 BMPString value according to CER, in
 * reverse order (VLT rather than TLV).
 *
 * A BMPString of 500 or less characters is encoded in primitive form.  All
 * other values are encoded in constructed form.  When encoding in constructed
 * form, if tagging is ASN1IMPL, the caller is responsible for encoding the EOC
 * (prior to calling this function), the tag (which must use constructed form),
 * and the length (which must be indefinite); if tagging is ASN1EXPL (and
 ...
 *                       successful.
 */
EXTERNBER int xe_cer16BitCharStr(OSCTXT* pctxt, Asn116BitCharString* object_p,
   ASN1TagType tagging, ASN1TAG tag);

/**
* This function will encode an ASN.1 UniversalString value according to CER, in
* reverse order (VLT rather than TLV).
*
* A UniversalString of 250 or less characters is encoded in primitive form.  All
* other values are encoded in constructed form.  When encoding in constructed
* form, if tagging is ASN1IMPL, the caller is responsible for encoding the EOC
* (prior to calling this function), the tag (which must use constructed form),
* and the length (which must be indefinite); if tagging is ASN1EXPL (and
...
*                       successful.
*/
EXTERNBER int xe_cer32BitCharStr(OSCTXT* pctxt, Asn132BitCharString* object_p,
   ASN1TagType tagging, ASN1TAG tag);
 ...
 * character string types. If the ASN1CANON or ASN1DER flag is set, this will
 * ensure the encoding complies with CER/DER.
 *
 * @param pctxt        Pointer to a context block structure. This provides a
 *                       storage area for the function to store all working
 *                       variables that must be maintained between function
 ...
 *                       for this type is added or not. Users will generally
 *                       always set this value to 'ASN1EXPL'.
 * @param object_p     Pointer to value to be encoded. The ASN1DynOBJID
 *                       structure contains a numids fields to hold the
 *                       number of subidentifiers and a pointer to an array
 *                       to hold the subidentifier values.
 * @return             Length of the encoded message component. A negative
 *                       status value will be returned if encoding is not
 *                       successful.
 */
EXTERNBER int xe_dynObjId
(OSCTXT* pctxt, ASN1DynOBJID *object_p, ASN1TagType tagging);
...
EXTERNBER int xe_enumInt8
(OSCTXT* pctxt, OSINT8 *object_p, ASN1TagType tagging);

EXTERNBER int xe_enumInt16
...
EXTERNBER int xe_enumUInt8
(OSCTXT* pctxt, OSUINT8 *object_p, ASN1TagType tagging);

EXTERNBER int xe_enumUInt16
...
/**
 * This function returns the built-in type name associated with a BER
 * UNIVERSAL tag.  If the tag is not UNIVERSAL or the ID code is unknown,
 * NULL is returned.
 *
 * @param tag          The tag to convert.
 * @return             Universal type name or NULL if unknown.
 */
EXTERNBER const char* berTagToTypeName (ASN1TAG tag);
