...
EXTRTMETHOD OSRTSocket::OSRTSocket () :
   mSocket(OSRTSOCKET_INVALID), mRetryCount(1), mOwner(TRUE)
{
   rtxSocketsInit ();
...
(OSRTSOCKET socket, OSBOOL ownership, int retryCount) :
   mSocket(socket), mStatus(0), mRetryCount(retryCount), mOwner(ownership)
{
   rtxSocketsInit ();
   mInitStatus = mStatus;
}
...
EXTRTMETHOD OSRTSocket::OSRTSocket (const OSRTSocket& socket) :
   ...
   mStatus(0), mRetryCount(socket.mRetryCount), mOwner(FALSE)
{
   rtxSocketsInit ();
}
