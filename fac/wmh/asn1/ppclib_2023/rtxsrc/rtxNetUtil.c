...
(OSRTNETCONN* pNetConn, OSRTNETURL* pNetURL, const char* url)
{
   ...
   if (s && s[1] == '/' && s[2] == '/') {
      OSSIZE urllen = strlen(url);
      if (urllen >= 4 && url[0] == 'h' && url[1] == 't' &&
          url[2] == 't' && url[3] == 'p') {
         pNetURL->protocol = (urllen >= 5 && url[4] == 's') ?
            OSRTNetHttps : OSRTNetHttp;
      }
      else if (urllen >= 3 && url[0] == 'f' && url[1] == 't' && url[2] == 'p') {
         pNetURL->protocol = OSRTNetFtp;
      }
      else {
         pNetURL->protocol = OSRTNetOtherP;
      }
      s += 3;
   }
   else {
      ...
      pNetURL->protocol = OSRTNetHttp;
   }

   /* See if there is an @ character in the URL, which means it contains a
      username and password.  */
   atSign = strchr (pNetURL->buffer, '@');
   if (!atSign) { /* No username and password */
      /* Parse domain name */
      pNetURL->domain = s;
      while (*s != '\0' && *s != '/' && *s != ':') s++;
      if (*s == ':') {
         ...
         *s++ = '\0';  /* null terminate domain name and advance pointer */
         ...
         pNetURL->port = (pNetURL->protocol == OSRTNetHttps) ? 443 : 80;
      }
      ...
      pNetURL->username = pNetURL->password = pNetURL->authstring = 0;
   }
   else { /* Username and password are present.  */
      /* Parse the username.  */
      pNetURL->username = s;
      while (*s != '\0' && *s != '/' && *s != ':') s++;
      if (*s == ':') *s++ = '\0'; /* null terminate username and advance pointer */

      /* Parse the password.  */
      pNetURL->password = s;
      while (*s != '\0' && *s != '/' && *s != '@') s++;
      if (*s == '@') *s++ = '\0'; /* null terminate password and advance pointer */

      pNetURL->authstring = 0; /* Will be set later */
...
int rtxNetInitConn (OSCTXT* pctxt, OSRTNETCONN* pNetConn, const char* url)
{
   ...
   OSCRTLMEMSET (pNetConn, 0, sizeof(OSRTNETCONN));
   pNetConn->pctxt = pctxt;
   pNetConn->socket = OSRTSOCKET_INVALID;
   pNetConn->url.protocol = OSRTNetNoProto;
...
int rtxNetConnect (OSRTNETCONN* pNetConn)
{
   ...
   RTDIAGSTRM4 (pctxt, "~D Connecting to %s:%i..\n", domain, port);
   ...
   for (i = 0; i < MAX_CONNECT_ATTEMPTS; i++) {
      ...
      if (0 != stat) {
         if (i + 1 == MAX_CONNECT_ATTEMPTS) {
            RTDIAGSTRM3
               (pctxt, "~D Connection failed, error code is %i\n", stat);
            return LOG_RTERR (pctxt, stat);
         }
      }
      else break;
   }
   RTDIAGSTRM2 (pctxt, "~D Connection established.\n");
