...
EXTRTMETHOD int rtxReadBytes (OSCTXT* pctxt, OSOCTET* pdata, size_t nocts)
{
   ...
   if (pctxt->buffer.byteIndex + nocts > pctxt->buffer.size) {
      ...
      if (0 != pStream) {
         ...
         if ((OSSIZE)stat < nocts ) return LOG_RTERRNEW(pctxt, RTERR_ENDOFFILE);
      }
      else
#endif
      ...
      int stat = (int) rtxStreamRead (pctxt, pdata, nocts);
      if (stat < 0 ) return LOG_RTERR(pctxt, stat);
      else if ((OSSIZE)stat < nocts)
         return LOG_RTERRNEW(pctxt, RTERR_ENDOFFILE);
      else return 0;
   }
#endif
   else {
