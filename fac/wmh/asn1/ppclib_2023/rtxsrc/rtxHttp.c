...
#if !defined(_OS_NOSOCKET) && !defined(_NO_STREAM)
...
/* Uncomment to enable diag tracing in this module
#define RTXHTTPDBG
*/
#ifdef RTXHTTPDBG
/* set to a negative value to simulate recv error */
static int g_recv_error = -1;

/* count on which operation should fail */
#define FAIL_COUNT 3
static int g_fail_count;

/* This is used in acLicenseTest.cpp */
void rtxHttpSetRecvError(int value) { g_recv_error = value; }

#define RTXHTTPDIAG1(pctxt,msg)   rtxDiagStream(pctxt,msg)
#define RTXHTTPDIAG2(pctxt,msg,a) rtxDiagStream(pctxt,msg,a)
#define RTXHTTPDIAGCHARS(pctxt,buf,nchars) \
rtxDiagStreamPrintChars(pctxt,buf,nchars)
#else
#define RTXHTTPDIAG1(pctxt,msg)
#define RTXHTTPDIAG2(pctxt,msg,a)
#define RTXHTTPDIAGCHARS(pctxt,buf,nchars)
#endif

int rtxHttpGetTls(OSCTXT* pctxt, const char* url, OSRTHttpContent* pContent,
                  tlsInit_t* f_tls_init, tlsClose_t* f_tls_close)
{
   OSRTNETCONN netConn;
...
#ifdef RTXHTTPDBG
   rtxDiagSetTraceLevel(pctxt, OSRTDiagDebug);
#endif
   ...
   if (0 != pNetConn->proxy.domain) {
      ret = rtxHttpSendConnectRequest(pNetConn);
      if (0 != ret) return LOG_RTERR(pctxt, ret);
   }

   if (pNetConn->url.protocol == OSRTNetHttps) {
      if (f_tls_init) {
         /* Setup secure connection over the socket. */
         ret = f_tls_init(pctxt, pNetConn->url.domain);
         RTXHTTPDIAG2(pctxt, "rtxHttpGetTls: f_tls_init returned %d\n", ret);
      }
      else {
         /* Secure connection required, but we can't make it. */
         rtxErrAddStrParm(pctxt, "https protocol");
   ...
   if (f_tls_init && pNetConn->url.protocol == OSRTNetHttps) {
      /* Close the secure connection. This will trigger closing the
         TLS connection, freeing the TLS stream, and restoring the
         socket stream as the context's active stream. */
      f_tls_close(pctxt);
   }
...
{
   return rtxHttpGetTls(pctxt, url, pContent, 0, 0);
}
   ...
   const char* method, const char* content, const char* contentType)
{
   ...
   if (pNetConn->url.protocol != OSRTNetHttp &&
       pNetConn->url.protocol != OSRTNetHttps) {
      return LOG_RTERR (pctxt, RTERR_INVPARAM);
   }

   rtxMemBufInit (pctxt, &httpBuffer, 512);

   rtxIntToCharStr (pNetConn->url.port, szPort, sizeof(szPort), 0);

   /* Create header */

   OSMBAPPENDSTR (&httpBuffer, method);
   OSMBAPPENDSTRL (&httpBuffer, " ");
   if (!pNetConn->proxy.bIsConnect) {
      ...
      OSMBAPPENDSTRL (&httpBuffer, ":");
      OSMBAPPENDSTRL (&httpBuffer, szPort);
   }
   OSMBAPPENDSTRL (&httpBuffer, " HTTP/1.1\r\nHost: ");
   OSMBAPPENDSTR (&httpBuffer, pNetConn->url.domain);

   if (pNetConn->url.port != 80 || pNetConn->proxy.bIsConnect) {
      OSMBAPPENDSTRL (&httpBuffer, ":");
      OSMBAPPENDSTRL (&httpBuffer, szPort);
   }
   ...
   /* If we're dealing with a proxy server that has a username and password,
      we should at this point have authstring valued in the proxy structure.
      In this case we need to add a Proxy-Authorization header.  */
   if (0 != pNetConn->proxy.authstring) {
      OSMBAPPENDSTRL (&httpBuffer, "Proxy-Authorization: Basic ");
      OSMBAPPENDSTRL (&httpBuffer, pNetConn->proxy.authstring);
      OSMBAPPENDSTRL (&httpBuffer, "\r\n");
   }

   /* The content is appended to the buffer before transmission.  Do not send
      it as a separate call to rtxStreamWrite().  At least one proxy server
      (FreeProxy) may try to send it to the web server if it comes in as a
   ...
   if (contentLen > 0) {
      ...
      OSMBAPPENDSTRL (&httpBuffer, "\r\n");
      OSMBAPPENDSTRL (&httpBuffer, "\r\n");
      rtxMemBufAppend (&httpBuffer, (const OSOCTET *)content, contentLen);
   }
   else {
      OSMBAPPENDSTRL (&httpBuffer, "\r\n");
   }
   ...
   RTXHTTPDIAG1 (pctxt, "HTTP header:\n");
   RTXHTTPDIAGCHARS (pctxt, (const char*)bufptr, buflen);
...
int rtxHttpSendGetRequest (OSRTNETCONN* pNetConn, const char* url)
{
   ...
   pNetConn->proxy.bIsConnect = FALSE;
   return rtxHttpSendRequest (pNetConn, "GET", 0, 0);
}
...
static int readHttpLine (OSCTXT* pctxt, char* lbuf, size_t lbufsiz)
{
   ...
   for (i = 0; i < lbufsiz; i++) {
      stat = rtxStreamRead (pctxt, (OSOCTET*)&lbuf[i], 1);
      if (1 != stat) {
         RTXHTTPDIAG2(pctxt, "rtxStreamRead status = %d\n", stat);
         RTXHTTPDIAGCHARS(pctxt, lbuf, i);
         RTXHTTPDIAG1(pctxt, "\n");
...
int rtxHttpRecvRespHdr (OSRTNETCONN* pNetConn, OSRTHttpHeader* pHeader)
{
   ...
   do {
      ...
      RTXHTTPDIAG2 (pctxt, "HTTP major version = %d\n", pHeader->majorVersion);
      RTXHTTPDIAG2 (pctxt, "HTTP minor version = %d\n", pHeader->minorVersion);
      ...
      RTXHTTPDIAG2 (pctxt, "HTTP status = %d\n", httpStat);
...
(OSRTNETCONN* pNetConn, OSRTHttpHeader* pHeader, OSRTHttpContent* pContent)
{
   ...
   if (pContent->length > 0) {
      ...
      lstat = rtxStreamRead (pctxt, pContent->data, pContent->length);
...
int rtxHttpSendConnectRequest(OSRTNETCONN* pNetConn)
{
   OSRTHttpHeader header;
   ...
   /* If we're dealing with a proxy that has a username and password, we need
      to create a Base 64 authentication string.  */
   if (pNetConn->proxy.username != 0) {
      const char* cleartext;

      /* username:password\0 */
      cleartext = rtxStrDynJoin (pctxt, pNetConn->proxy.username, ":",
                                 pNetConn->proxy.password, 0, 0);

      ret = rtxBase64EncodeData (pctxt, cleartext, strlen (cleartext),
                                 (OSOCTET**)&pNetConn->proxy.authstring);

      rtxMemFreePtr (pctxt, cleartext);
      if (ret < 0) return LOG_RTERR (pctxt, RTERR_INVFORMAT);
   }

   pNetConn->proxy.bIsConnect = TRUE;
   ret = rtxHttpSendRequest (pNetConn, "CONNECT", 0, 0);
...
#endif /* !defined(_OS_NOSOCKET) %% !defined(_NO_STREAM) */
