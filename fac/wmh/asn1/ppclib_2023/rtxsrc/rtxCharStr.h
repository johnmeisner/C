 ...
 * @param str1         Pointer to first string to compare.
 * @param str2         Pointer to second string to compare.
 * @param count        Number of characters to compare, at most.
 * @return             0 if strings are equal, non-zero if not.
 */
...
/**
 * This function creates a duplicate copy of up to the given number of
 * characters in a string.  The string does not need to be null-terminated.
 * Memory is allocated for the target string using the rtxMemAlloc
 * function.  The string is then copied into this memory block.
 * It is similar to the C \c strndup function except more secure because
 * it checks for buffer overrun.
 *
 * @param pctxt        Pointer to a standard context structure.
 * @param src          Pointer to null-terminated string to copy.
...
/**
 * This function will validate a constrained character string by checking to
 * see if all the characters in the given string are contained within the
 ...
 *                       validated.
 * @param pCharSet     A pointer to a null-terminated C character string
 *                       containing the character set to validate against.
 * @return             Status of the validation attempt. A negative status
 *                       value will be returned if validation is not
 *                       successful.
 */
EXTERNRT int rtxValidateConstrainedStr(OSCTXT *pctxt, const char *pvalue,
                                       const char *pCharSet);
...
/**
 * This function converts up to the given number of digits from the given
 * character string to a signed 32-bit integer value.  It consumes all
 * leading whitespace before the digits start. It then comsumes digits
 * until either the number of digits is reached or a non-digit character is
 * encountered.
 *
 * @param cstr         Character string to convert.
