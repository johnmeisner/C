...
static long socketRead (OSRTSTREAM* pStream, OSOCTET* pbuffer, size_t bufSize)
{
   ...
   if (pStream->flags & OSRTSTRMF_HEXTEXT) {
      ...
      /* Keep trying to read until we don't get an RTERR_WOULDBLOCK error. */
      for (;;) {
      ...
      for (;;) {
         ...
         if (sdesc->readTimeoutSecs > 0) {
            ...
            len = rtxSocketRecv (sdesc->socket, pbuffer, bufSize);
         }

         if (len != RTERR_WOULDBLOCK) return len;
      }
   }
}
