...
class EXTRTCLASS OSRTSocket {
 protected:
   ...
   int        mInitStatus;
   int        mStatus;
   int        mRetryCount;  ///< number of time to retry socker connect
 ...
 public:
    ...
    * @param retryCount   Number of times to retry a socket connect operation.
    */
   EXTRTMETHOD OSRTSocket
      (OSRTSOCKET socket, OSBOOL ownership = FALSE, int retryCount = 1);
    ...
    *                       occur. Use OSRTSocket::getStatus method to obtain
    *                       error code.
    * @see               ::rtxSocketAccept
    */
    ...
    * unconnected socket before subsequent calls to the OSRTSocket::connect
    * or OSRTSocket::listen methods.
    *
    ...
    * OSRTSocket::listen methods.  This version of the method allows a URL to
    * be used instead of address and port number.
    *
    * @param   url        Univeral resource locator (URL) string.
    * @return             Completion status of operation:
    *                       - 0 = success,
    *                       - negative return value is error.
    * @see                ::rtxSocketBind
    ...
    *                       - 0 = success,
    *                       - negative return value is error.
    * @see                            ::rtxSocketConnect
    */
   EXTRTMETHOD int connect (const char* host, int port);

   /**
    * This method establishes a connection to this socket. It is
    * similar to the socketConnect method except that it will only
   ...
   /**
    * This method sets the socket connect retry count.  The connect operation
    * will be retried this many times if the operation fails.  By default,
    * the connect operation is not retried.
    *
    * @param value       Retry count.
    */
   inline void setRetryCount (int value) { mRetryCount = value; }
