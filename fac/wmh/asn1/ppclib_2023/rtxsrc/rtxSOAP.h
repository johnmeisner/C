...
typedef struct OSSOAPCONN {
   ...
   int          responseStatusCode; /* status code to send in HTTP response */
   OSUINT32     recvTimeoutSecs;
} OSSOAPCONN;
...
/**
 * This function sends a complete HTTP POST request to a SOAP
 * connection.  The request is stored in the XBinder context buffer.
 * If an XML encode operation was just completed, then calling this
...
/**
 * This function sets the read timeout value to the given
 * number of seconds.  Any read operation attempted on the SOAP
 * connection will timeout after this period of time if no data
 * is received.
 *
 * @param pctxt        Pointer to a context structure variable that has
 *                       been initialized for stream operations.
 * @param nsecs        Number of seconds to wait before timing out.
