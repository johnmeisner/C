...
EXTRTMETHOD int rtxBigIntEnsureCapacity(OSCTXT* pctxt, OSBigInt* pInt,
                                       OSSIZE capacity)
{
...
EXTRTMETHOD int rtxBigIntSetInt64
...
EXTRTMETHOD int rtxBigIntSetUInt64
...
EXTRTMETHOD int rtxBigIntSetBytesSigned
(OSCTXT* pCtxt, OSBigInt* pInt, OSOCTET* value, OSSIZE vallen)
{
   ...
   if (*p & 0x80) { /* negative case */
      ...
      return rtxBigIntSetBytesUnsigned(pCtxt, pInt, value, vallen);
   }
...
EXTRTMETHOD int rtxBigIntSetBytesUnsigned
(OSCTXT* pCtxt, OSBigInt* pInt, OSOCTET* value, OSSIZE vallen)
{
   unsigned int i = 0;
   OSOCTET *p = value;
   ...
   OSCRTLSAFEMEMCPY (pInt->mag, pInt->allocated, p, pInt->numocts);

   /* this was done at the start of the function
   rtxBigIntRemoveLeadingZeroBytes (pInt);
   */
   return 0;
}
...
EXTRTMETHOD OSSIZE rtxBigIntGetDataLen (const OSBigInt* pInt)
{
...
EXTRTMETHOD int rtxBigIntGetData
...
EXTRTMETHOD OSSIZE rtxBigIntDigitsNum (const OSBigInt* pInt, int radix)
{
...
EXTRTMETHOD int rtxBigIntFastCopy
(OSCTXT* pCtxt, const OSBigInt* pSrc, OSBigInt* pDst)
{
...
EXTRTMETHOD OSBOOL rtxBigIntToReal(const OSBigInt* pSrc, OSREAL* pvalue)
{
   OSREAL result = 0.0;
...
EXTRTMETHOD int rtxBigIntToString
...
EXTRTMETHOD char* rtxBigIntToDynStr
(OSCTXT* pctxt, const OSBigInt* pInt, int radix)
{
   /* Compute approximate size for the string.  This should be worst case. */
   char* str;
   OSSIZE charsPerByte, len;
   switch (radix) {
   case 2: charsPerByte = 8; break;
   case 10: charsPerByte = 3; break; /* worst case */
   case 16: charsPerByte = 2; break;
   default: return 0; /* not supported */
   }
   len = (charsPerByte * rtxBigIntGetDataLen(pInt)) + 4;

   /* Allocate memory and do conversion to string */
   str = rtxMemAlloc(pctxt, len+1);
   if (0 != str) {
      int ret = rtxBigIntToString(pctxt, pInt, radix, str, len);
      if (0 != ret) {
         rtxMemFreePtr(pctxt, str);
         str = 0;
      }
   }
...
EXTRTMETHOD int rtxBigIntPrint
 ...
 * less than, equal to, or greater than arg2.
 */
EXTRTMETHOD int rtxBigIntStrCompare
