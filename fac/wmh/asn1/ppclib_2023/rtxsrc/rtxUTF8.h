...
/**
 * This function converts a UTF-8 string to a Unicode string (UTF-32). The
 * Unicode string is stored as an array of 32-bit characters (unsigned
 * integers).
 *
 * @param pctxt        A pointer to a context structure.
 * @param inbuf        UTF-8 string to convert.
 ...
 *                       - negative return value is error.
 */
EXTERNRT long rtxUTF8ToUnicode32 (OSCTXT* pctxt, const OSUTF8CHAR* inbuf,
                                 OS32BITCHAR* outbuf, size_t outbufsiz);
 ...
 * @param pnchars      Pointer to integer to receive number of chars (the size
 *                     of *ppdata array).
 * @return             Status: 0 = OK, negative value = error
 */
