...
/**
 * @file rtxBitString.h
 * Contains utility functions for setting, clearing, and testing bits at any
 * position in an arbitrarily sized array of bytes.
 */
 ...
 * @return             If successful, returns the previous state of the bit. If
 *                       bit was previously set, the return value is positive.
 *                       If bit was not previously set, the return value is
 *                       zero. Otherwise, return value is an error code:
 ...
 * @param pBits       Pointer to the octets of the bit string.
 * @param numbits     The number of bits in the bit string.
 * @return            Zero if the operation is successful, or a negative status
 *                      code if the operation fails.
 */
EXTERNRT int rtxZeroUnusedBits (OSOCTET* pBits, OSSIZE numbits);

/**
 * This function checks to see if unused bits at the end of the given bit
 * string are zero.
 *
 * @param pBits       Pointer to the octets of the bit string.
 * @param numbits     The number of bits in the bit string.
 * @return            Zero if the operation is successful, or a negative status
 *                      code if the operation fails.  RTERR_INVBINS will be
 *                      returned if bits at end are not zero.
 */
