...
void _rtxMemHeapFreeElem (void** ppvMemHeap, OSMemElemDescr* pElem)
{
   ...
   if (ISFREE (pNextElem)) {
      if (ISFREE (pPrevElem)) {
         /* pElem is bordered by free elements.
            We will merge the three elements into one, new, bigger element.
            To do this, we'll resize pPrevElem to cover pPrevElem, pElem, and
            pNextElem.  We also have to update the free list.  Within the free
            list, we'll basically move pPrevElem to where pNextElem is now,
            and get rid of pNextElem, though that isn't precisely what happens,
            because the free list is really a list of link structures, not of
            elements, and by changing the size of pPrevElem, we're changing
            where its link structure is located.  The following explains this
            in more detail.

            First, note that the free list link structure appears at the end of
            each element, and that the link structure holds offsets used to
            locate other link structures (so, in essence link structs point to
            link structs).  Let element.link be the notation for the link
            structure for a given element.

            Let P, E, N represent the elements which are being merged into P.
            Let P' be the new, combined element.  Note that P.link and P'.link
            are NOT the same strutures.  Rather, P'.link IS the same structure
            as N.link (this is because the link struct is at the end and P is
            being resized).

            Suppose the free list looks like ..., X, P, Y, ..., with N being
            in this list somewhere.  This means, for example, that
            X.link.next -> P.link. Either X == N, or Y == N, or neither of
            these.

            If Q.link -> N.link before the resize operation, then
            Q.link -> P'.link after the resize operation.  We can think of this
            as putting P' in place of N in the free list.  But then, we don't
            need the original entry for P any more, and indeed after the
            resize operation, P.link is invalid.  Consequently, we change:
               X.link.nextFree = P.link.nextFree and
               Y.link.prevFree = P.link.prevFree,
            effectively removing P (or P.link) from the free link list.
         */
   ...
   TRACEMEMBLK_FREELIST(pMemBlk, "post-elem-free");
...
EXTRTMETHOD int rtxMemHeapAutoPtrUnref (void** ppvMemHeap, void* mem_p)
{
   ...
   if (rtxMemInvalidPtr(pMemHeap, mem_p ) ||
      !rtxMemHeapCheckCode (pMemHeap, pElem))
   {
      /* rtxMemHeapInvalidPtrHook (ppvMemHeap, mem_p); */
