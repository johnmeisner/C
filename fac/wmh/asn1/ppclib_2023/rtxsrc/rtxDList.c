...
EXTRTMETHOD int rtxDListToPointerArray
(struct OSCTXT* pctxt, OSRTDList* pList, void*** ppArray,
   OSSIZE* pElemCount)
{
   OSRTDListNode* pNode;
   ...
   if (arrayCount < pList->count) {
      *ppArray = rtxMemAlloc(pctxt, pList->count * sizeof(void*));
      if (*ppArray == NULL) return LOG_RTERRNEW(pctxt, RTERR_NOMEM);
   ...
   for (pNode = pList->head; pNode != 0; pNode = pNode->next, i += 1) {
      (*ppArray)[i] = (void*) pNode->data;
   }
