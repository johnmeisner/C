...
#define RT_MH_TRACE_FREELIST 0x200  /* enable printing of the free element list
 ...
 * C run-time. Special algorithms are used for allocation and
...
EXTERNRT void* rtxMemHeapAlloc (void** ppvMemHeap, OSSIZE nbytes);
EXTERNRT void* rtxMemHeapAllocZ (void** ppvMemHeap, OSSIZE nbytes);
EXTERNRT void* rtxMemHeapSysAlloc (void** ppvMemHeap, OSSIZE nbytes);
EXTERNRT void* rtxMemHeapSysAllocZ (void** ppvMemHeap, OSSIZE nbytes);
...
/* Internal runtime use only.
   This can only be used with a static memory heap.
   It will reallocate the last memory block if the given block of the
   given size appears right at the end of the static memory heap.
   @param ppvMemHeap the heap
   @param mem_p Pointer to block to resize
   @param oldsize The previously allocated size of the block.
   @param newsize The desired new size of the block.
*/
EXTERNRT void* rtxMemHeapReallocStatic
(void** ppvMemHeap, void* mem_p, OSSIZE oldsize, OSSIZE newsize);

EXTERNRT void* rtxMemHeapRealloc
(void** ppvMemHeap, void* mem_p, OSSIZE nbytes_);
...
EXTERNRT void* rtxMemNewArray (OSSIZE nbytes);
EXTERNRT void* rtxMemNewArrayZ (OSSIZE nbytes);
...
/**
 * Print the same details about the memory heap as rtxMemHeapPrint but add
 * deatils about the free memory list.
 */
EXTERNRT void rtxMemHeapPrintWithFree (void **ppvMemHeap);

/**
 * This function creates a standard memory heap.  It is invoked internally
 * from the rtxInitContext function to create the heap in the context.
 *
 * @param ppvMemHeap Pointer-to-pointer to variable to receive created object.
 * @return Status of the creation operation: 0 = success or RTERR-NOMEM if
 *         no memory available.
 */
EXTERNRT int rtxMemHeapCreate (void** ppvMemHeap);

/**
 * This function creates a standard memory heap and sets the low-level
 * memory functions to the specified values.  It is invoked internally
 * from the rtxInitContextExt function to create the heap in the context.
 *
 * @param ppvMemHeap Pointer-to-pointer to variable to receive created object.
 * @param malloc_func Pointer to memory allocation function.
 * @param realloc_func Pointer to memory reallocation function.
 * @param free_func Pointer to memory free function.
 * @return Status of the creation operation: 0 = success or RTERR-NOMEM if
 *         no memory available.
 */
...
/**
 * This function creates a static memory heap.  All allocations are done
 * from the static block of memory that is provided.  It is much faster
 * than the standard management but has some limitations such as the
 * inability to free individual pointer values.  All memory in the
 * block must be freed at once.
 *
 * @param ppvMemHeap Pointer-to-pointer to variable to receive created object.
 * @param pmem Pointer to static memory block to use for allocations.
 * @param memsize Sizeof the memory block in bytes.
 * @return Status of the creation operation: 0 = success or RTERR-NOMEM if
 *         no memory available.
 */
EXTERNRT int rtxMemStaticHeapCreate
(void **ppvMemHeap, void* pmem, OSSIZE memsize) ;
...
/**
 * Print memory heap structure to stderr, the same as rtxMemPrint, but
 * add details about the memory free list.
 *
 * @param pctxt - Pointer to a context block
 */
#define rtxMemPrintWithFree(pctxt) \
rtxMemHeapPrintWithFree(&(pctxt)->pMemHeap)
