...
/**
 * This function will return the node pointer of the first non-null data item
 * within the list or NULL if there is no node that has non-null data.
 *
 * @param pList         A pointer to a linked list structure.
 * @return              A pointer to an allocated linked list node structure.
 */
EXTERNRT OSRTDListNode*
rtxDListFindFirstData (const OSRTDList* pList);
...
/**
 * This function converts a doubly linked list of *T to a dynamically (or
 * pre-allocated) array of T.  The values pointed to are copied to the
 * array.  Note: if you have an array of strings to copy to, you should use
 * rtxDListToPointerArray.
 *
 * @param pctxt         A pointer to a context structure.
 * @param pList         A pointer to a linked list structure.
...
/**
 * This function converts a doubly linked list of *T to a dynamically (or
 * pre-allocated) array of *T.  The pointers are copied from the list to the
 * array.
 *
 * @param pctxt         A pointer to a context structure.
 * @param pList         A pointer to a linked list structure.
 * @param ppArray       A pointer to a pointer to the array of pointers.
 * @param pElemCount    A pointer to the number of elements already
 *                        allocated in \c ppArray.  If pElements is NULL, or
 *                        pElements is less than the number of nodes
 *                        in the list, then a new array is allocated
