...
{
   /* Check-in ifloat license if flag set */
   if (0 != (pctxt->flags & OSLICCHECKIN)) {
      LCHECKIN(pctxt);
   }
...
EXTRTMETHOD int rtxCtxtMarkBitPos (OSCTXT* pctxt, OSSIZE* ppos)
{
   OSSIZE bitPos;
   OSSIZE byteIndex;
#ifndef _NO_STREAM
   byteIndex = (OSRTISSTREAM(pctxt)) ?
      pctxt->pStream->bytesProcessed : 0;
   ...
   byteIndex = pctxt->buffer.byteIndex;
#endif /* _NO_STREAM */
   bitPos = ((byteIndex * 8) + (8 - pctxt->buffer.bitOffset));
   if (bitPos < byteIndex) {
      *ppos = 0;
      return RTERR_TOOBIG;
   }
...
EXTRTMETHOD int rtxCtxtResetToBitPos (OSCTXT* pctxt, OSSIZE pos)
{
   return rtxCtxtSetBitOffset(pctxt, pos);
}
...
EXTRTMETHOD int rtxCtxtPushElemNameCopy
(OSCTXT* pctxt, const OSUTF8CHAR* elemName)
{
   /* Add a copy of name to element name stack in context.  This must be
      popped using rtxCtxtPopElemNameCopy to avoid a memory leak. */
   OSUTF8CHAR* elemNameCopy = rtxUTF8Strdup (pctxt, elemName);
   if (0 != elemNameCopy) {
      return rtxDListAppend
         (pctxt, &pctxt->elemNameStack, (void*)elemNameCopy) ?
         0 : RTERR_NOMEM;
   }
   return RTERR_NOMEM;
}
...
EXTRTMETHOD void rtxCtxtPopElemNameCopy (OSCTXT* pctxt)
{
   /* Note: name can't be returned because it will be freed */
   if (pctxt->elemNameStack.count > 0) {
      rtxMemFreePtr (pctxt, pctxt->elemNameStack.tail->data);
      rtxDListFreeNode
         (pctxt, &pctxt->elemNameStack, pctxt->elemNameStack.tail);
   }
}
