...
/**
 * This function reads the current system date and time and returns it as
 * a formatted string.  The format is that returned by the asctime system
 * function.
 *
 * @param buffer       Character buffer to receive string. Buffer should be
 *                       at least 32 characters in size.
 * @param bufsize      Size of buffer.  If string will not fit in buffer, an
 *                       error is returned.
 * @param local        True for local time, false for GMT.
 * @return             Completion status of operation:
 *                     - 0 in case success
 *                     - negative in case failure
 */
...
/**
 * This function gets GM time.  It provides a platform independent
 * abstraction of the the C RTL gmtime function.
 *
 * @param pvalue    A pointer to tm structure to receive time value.
 * @param timeMs    Time value to be converted to GMT.
 * @return          Completion status of operation:
 *                  - 0(RT_OK) = success,
 *                  - negative return value is error.
 */
...
/**
 * This function gets local time.  It provides a platform independent
 * abstraction of the the C RTL localtime function.
 *
 * @param pvalue    A pointer to tm structure to receive local time value.
 * @param timeMs    Time value to be converted to local time.
 * @return          Completion status of operation:
 *                  - 0(RT_OK) = success,
 *                  - negative return value is error.
 */
...
/**
 * This function returns a string representation of the given date/time
 * structure.  It is similar to the std::asctime function except uses
 * the secure version of asctime (asctime_s) for Windows.
 *
 * @param buffer      Character array into which to write characetr data.
 * @param bufsize     Size of the character buffer.
 * @param pvalue      Pointer value to convert.
