...
#include <errno.h>
/* extern int errno;        //okay for older UNIXes but not for newer */
#endif
...
#define RETURN_RTERR(pctxt, stat) \
...
#define RETURN_RTERR(pctxt, stat) return stat
...
/**
 * This logs an error to a global context and frees a memory pointer allocated
 * for encoding or decoding.
 *
 * @param   ctxt_p      A pointer to the main context data structure.
 * @param   stat        The error status.
 * @param   mem_p       The memory pointer allocated for encoding/decoding.
 *
 ...
 *                       allocated dynamic buffer.  If pBuf is not null,
 *                       this is expcted to be set and hold the size
 *                       of the buffer.
...
/**
 * This function returns error message text in a memory buffer. It only
 * returns the formatted error message, not the error status nor stack
 * trace information.  Memory is dynamically allocated using the
 * rtxMemAlloc function.  It should be freed using rtxMemFreePtr or
 * it will be freed when the context is freed.
 *
 * @param pctxt        A pointer to a context structure.
 * @return             A pointer to a buffer with error text. Dynamic
 *                     memory will be allocated for this buffer using
 *                     rtxMemAlloc.  It should be freed using rtxMemFreePtr.
 */
EXTERNRT char* rtxErrGetMsgText (OSCTXT* pctxt);

/**
 * This function returns error message text in a static memory buffer. It
 * only returns the formatted error message, not the error status nor stack
 * trace information.  If the formatted text will not fit in the buffer,
 * it is truncated.
 *
 * @param pctxt        A pointer to a context structure.
 * @param pbuf         Pointer to a destination buffer to receive text.
 ...
 * The new record will have fatal == TRUE.
 *
 * @param pctxt        A pointer to a context structure.
 ...
 * This function is used to reset the error state recorded in the context to
 * successful. It is used in the generated code in places where automatic error
 * correction can be done.  This version differs from rtxErrReset in that it
 * only resets error information within the context and not the element name
 ...
 * error conditions.
 * If no error has been logged, a new log entry is created. The new entry will
 * have fatal = TRUE.
 *
 * @param pctxt        A pointer to a context structure.
 ...
 * error conditions. It always allocates new error record with fatal = TRUE.
 *
 * @param pctxt        A pointer to a context structure.
 * @param status       The error status code from \c rtxErrCodes.h
 * @param module       The C source file in which the error occurred.
 ...
 * Non-fatal errors are ignored by rtxErrGetStatus but are otherwise treated
 * the same as any other error. This is useful when recording, for example,
 * canonical violations, which are errors but which don't force an operation to
 * abort.
 */
 ...
 * are all the same error.  If there are at least two different error conditions,
 * the function will return RTERR_MULTIPLE.  If there are no error conditions,
...
/**
 * This function returns the total number of error records, including non-fatal
 * errors.
 *
 * @param pctxt        A pointer to a context structure.
 ...
 * examines the error list to see how many fatal errors were logged.  If
 * none, OK (zero) is returned; if one, then the status value in that
 ...
 * Non-fatal errors are entirely ignored.
 *
 * @param pctxt        A pointer to a context structure.
 ...
 * Any error information that may have existed in the destination
 * context prior to the operation is lost.
 *
 ...
 * Error information is added to what previously existed in the
 * destination context.
 *
 * @param pDestCtxt    Pointer to destination context structure to
 *                       receive the copied error information.
...
/**
 * This function create an 'invalid option' error (RTERR_INVOPT)
 * in the context using an unsigned integer parameter.
 *
 * @param pctxt        Pointer to context structure.
 * @param ident        Identifier which was found to be invalid.
 */
