...
/* These macros do not null-terminate the appended string */
...
/* This macro will null-terminate the appended string */
#define OSMBAPPENDSTRZ(pmb,str) \
rtxMemBufAppend(pmb,(OSOCTET*)str,OSCRTLSTRLEN(str)+1)
 ...
 * the buffer using rtxMemBufAppend) and a non-zero segsize was specified,
 * a dynamic buffer will be allocated and all data copied to the new buffer.
 *
 ...
 *                       expanded in case it is full. If 0, when expansion is
 *                       needed, an RTERR_NOMEM error will result.
 *
 */
 ...
 *
 * @param pMemBuf      A pointer to a memory buffer structure.
 * @param value        Boolean indicating system memory management to be used.
