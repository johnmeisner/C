...
EXTRTMETHOD void rtxBigFloatInit (OSBigFloat* pFloat)
{
   pFloat->base10 = 1;
   pFloat->exponent = 0;
...
(OSCTXT* pctxt, OSBigFloat* pFloat, const char* value, OSSIZE vlen)
{
   ...
   if ( endMantissa < vlen )
   {
      ...
      ret = rtxCharStrToInt64(value + endMantissa + 1, &exponent);
      if ( ret != 0 ) return LOG_RTERR(pctxt, ret);
...
EXTRTMETHOD OSSIZE rtxBigFloatStringSize(OSBigFloat* pFloat)
{
   if ( pFloat->base10 ) {
      OSSIZE result = 0;
...
EXTRTMETHOD OSBOOL rtxBigFloatToReal(const OSBigFloat* pFloat, OSREAL* pvalue)
{
   OSREAL result;
   OSBOOL ret;
   OSREAL powerOf2;
   ...
   if ( pFloat->exponent >= DBL_MAX_EXP )
   {
      ...
      OSINT64 exponent = pFloat->exponent;

      if ( exponent < DBL_MIN_EXP && DBL_MIN_EXP - exponent <= DBL_MAX_EXP)
      {
         /* Adjust exponent to be >= DBL_MIN_EXP, if possible. This is
            this is preferrable to adjusting it to be >= SMALLEST_SUBNORMAL_EXP,
            because subnormals might not be supported.
         */
         OSINT64 expAdd = DBL_MIN_EXP - exponent;
                                             /* 0 <= expAdd <= DBL_MAX_EXP */
         double divideBy = pow(2, (double)expAdd);
         if (fmod(result, divideBy) == 0) {
            /* The mantissa is divisible by divideBy.  We can adjust
               the mantissa and exponent to bring the exponent in range. */
            exponent += expAdd;
            result = result / divideBy;
         }
         /* else we can't fix the exponent to be >= DBL_MIN_EXP.
            We'll try to get it >= SMALLEST_SUBNORMAL_EXP below. */
      }

      if (exponent < SMALLEST_SUBNORMAL_EXP &&
            SMALLEST_SUBNORMAL_EXP - exponent <= DBL_MAX_EXP)
      {
         /* Adjust exponent to be >= SMALLEST_SUBNORMAL_EXP, if possible.
         */
         OSINT64 expAdd = SMALLEST_SUBNORMAL_EXP - exponent;
                                             /* 0 <= expAdd <= DBL_MAX_EXP */
         double divideBy = pow(2, (double)expAdd);
         if (fmod(result, divideBy) == 0) {
            /* The mantissa is divisible by divideBy.  We can adjust
            ...
            /* we can't fix the exponent to be >= SMALLEST_SUBNORMAL_EXP.
               The value is certainly not representable, even if subnormals are
               supported.
            */
            return FALSE;
         }
      ...
      /* 2^exponent won't overflow because exponent < DBL_MAX_EXP, but it
         could underflow if subnormals are not supported and
         exponent < DBL_MIN_EXP, or if subnormals are supported but
         exponent < SMALLEST_SUBNORMAL_EXP.
      */
      powerOf2 = pow(2, (double) exponent);
      if (powerOf2 == 0) return FALSE;    /* underflow */
...
EXTRTMETHOD int rtxBigFloatToString
      ...
      if ( fitsInReal ) {
         ...
         */
         ...
         for(octIdx = 0; octIdx < pFloat->mantissa.numocts; octIdx++)
         {
            ...
            b = octet >> 4;
            NIBBLETOUCHEXCHAR (b, str[idx]);   /* high nibble */
            idx++;

            b = octet & 0x0F;
            NIBBLETOUCHEXCHAR (b, str[idx]);   /* low nibble */
            idx++;
         }
...
EXTRTMETHOD void rtxBigFloatFree (OSCTXT* pCtxt, OSBigFloat* pFloat)
{
   if (pFloat == 0) return;
