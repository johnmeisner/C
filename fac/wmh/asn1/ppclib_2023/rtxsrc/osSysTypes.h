...
#if defined(__sun) || defined(NOCPP11)
#define nullptr NULL
#endif
...
/* Note: OSSIZE_MAX as defined here should NOT be used in preprocessor
   conditional expressions (e.g. #if).

   C99 says that conditional expressions in preprocessor diretives shall not
   contain casts, which it does (but evidently Visual Studio ignores this).
   C99 also says that the preprocessor treats signed/unsigned integer values as
   intmax_t/uintmax_t, which means the preprocessor sees 64-bit values and sees
   OSSIZE_MAX == 0xFFFFFFFFFFFFFFFFULL.
*/
#define OSSIZE_MAX      OSNULLINDEX

#if SIZE_MAX == 0xFFFFFFFFUL
   #define OSSIZE_IS_32BITS
#elif SIZE_MAX == 0xFFFFFFFFFFFFFFFFULL
   #define OSSIZE_IS_64BITS
#endif
