...
/**
 * Invalid parameter passed to a function or method.  This status code is
...
/**
 * Invalid bit string error.  This error is raised when a bit string is
 * decoded that contains bits that have not been set to zero.
 */
...
/**
 * No codec. This error is returned when the user has configured ASN1C to not
 * generate a decoder for a type, and then that type appears in an encoding.
 */
