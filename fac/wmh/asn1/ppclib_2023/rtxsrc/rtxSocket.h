 ...
 *                     RTERR_WOULDBLOCK is returned if the socket is
 *                     non-blocking and 0 bytes were available without blocking.
 */
 ...
 * It is used to read incoming data on sockets. The socket must be connected
 * before calling this function. If no data is available within the given
 * timeout period, an error is returned. See description of 'recv' socket
 ...
 * @param pbuf         Pointer to the buffer for the incoming data.
 * @param bufsize      Length of the buffer.
 * @oaram secs         Amount of time to wait, in seconds, for data
 *                       to be received.
 * @return             If no error occurs, returns the number of bytes
 *                       received. Otherwise, the negative value is error code.
 */
EXTERNRT int rtxSocketRecvTimed
(OSRTSOCKET socket, OSOCTET* pbuf, size_t bufsize, OSUINT32 secs);
