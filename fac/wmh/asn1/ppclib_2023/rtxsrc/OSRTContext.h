...
/*! \mainpage C++ Common Runtime Library Classes
 *
...
class EXTRTCLASS OSRTContext {
 ...
 public:
   ...
   /**
    * The memAllocZ method allocates and zeroes memory using the C runtime memory
    * management functions. The memory is tracked in the underlying context
    * structure. When both this OSXSDGlobalElement derived control class
    * object and the message buffer object are destroyed, this memory will
    * be freed.
    ...
    * @param numocts      - Number of bytes of memory to allocate
    */
   inline void* memAllocZ (size_t numocts) {
      return rtxMemAllocZ (&mCtxt, numocts);
   }
...
/**
 * Element name guard class.  This class pushes and pops element names
 * from the element name stack within the context for diagnostic and
 * error logging.
 */
class EXTRTCLASS OSRTElemNameGuard {
protected:
   OSCTXT* mpCtxt;

public:
   OSRTElemNameGuard (OSCTXT* pctxt, const char* elemName) : mpCtxt(pctxt)
   {
      rtxCtxtPushElemName (mpCtxt, OSUTF8(elemName));
   }
   ~OSRTElemNameGuard()
   {
      rtxCtxtPopElemName (mpCtxt);
   }
} ;
