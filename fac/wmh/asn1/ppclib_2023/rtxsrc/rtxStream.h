 ...
 *
 * The read function may read fewer bytes than requested, but it will read
 * at least one byte (assuming nocts > 0) unless EOF is reached or there is an
 * error.  Thus, if nocts > 0, this will return 0 only if EOF is reached.
 * Reaching EOF shall not be considered an error.
 *
 * @param pStream Identifies the stream control structure.
 * @param pbuffer is the buffer into which bytes shall be read.
 * @param nocts is the number of bytes to be read; the buffer must be at least
 *    this large.
 * @return The number of bytes read (possibly 0), or a negative error code.
 */
typedef long (*OSRTStreamReadProc)
   (struct OSRTSTREAM* pStream, OSOCTET* pbuffer, size_t nocts);
...
typedef struct OSRTSTREAM {
   ...
   size_t bufsize;              /**< physical size of pctxt->buffer.data
                                       buffer. pctxt->buffer.size represents
                                       the logical size - the amount of actual
 ...
 * and return, an error.
 *
 * Any bytes read from the stream will be sent to the capture buffer, if there
 * is one.
 *
...
/**
 * This function initializes a stream to use the context memory buffer for
 * stream buffering.
 *
 * @param pctxt        Pointer to context structure variable, for which stream
 *                       to be initialized.
...
/**
 * This function removes the use of a context memory buffer from a
 * stream.
 *
 * @param pctxt        Pointer to context structure variable which is
 *                       assumed to contain an initialized stream with
 *                       context buffering enabled.
 *
 * @return             Completion status of operation: 0 = success, negative
 *                       return value is error.
 */
 ...
 *
 * This function blocks until nocts of data are read, end of file is detected,
 * or an error occurs.  EOF is not treated as an error.
 *
 * @param pctxt        Pointer to a context structure variable which has been
 ...
 * @return             The total number of octets read into pbuffer, or
 *                       negative value with error code if an error occurs.
 */
EXTERNRT long rtxStreamRead
   (OSCTXT* pctxt, OSOCTET* pbuffer, size_t nocts);

/** rtxStreamBlockingRead is deprecated.  Use rtxStreamRead. */
#define rtxStreamBlockingRead rtxStreamRead

EXTERNRT long rtxStreamReadDirect
(OSCTXT* pctxt, OSOCTET* pbuffer, OSSIZE bufSize);


/**
 * This function skips over and discards the specified amount of data octets
 * from this input stream.  An attempt to skip past the end of the input is an
 * error. (This skips bytes of input, which may be held in a buffer, so it does
 * not necessarily skip bytes in the underlying stream.  Also,
 * pctxt->buffer.bitOffset is irrelevant to this function.)
 *
 * @param pctxt        Pointer to a context structure variable which has been
 *                       initialized for stream operations via a call to
 *                       ::rtxStreamInit.
