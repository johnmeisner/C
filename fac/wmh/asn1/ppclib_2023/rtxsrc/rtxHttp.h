...
typedef int tlsInit_t(OSCTXT* pctxt, const char* serverName);
typedef int tlsClose_t(OSCTXT* pctxt);
...
/**
 * TLS-enabled version of rtxHttpGet.  For internal use only.  This optionally
 * uses TLS.
 *
 * This exists to avoid making the runtime dependent on TLS code.
 *
 * @param f_tls_init If null, the connection does not use TLS.  If the url
 *    specifies https and f_tls_init is null, this returns an error.
 *    If not null and the url specifies https, then this uses f_tls_init
 *    to create a TLS stream on top of the socket stram created by this
 *    function.  If the url does not specify https, this argument is ignored.
 * @param f_tls_close Must be not null if and only if f_tls_init is not null.
 *    This is the function for tearing down a TLS connection.
 */
 ...
 * If the header does not indicate any additional content, the function does
 * nothing.
 *
 * @param pNetConn - Pointer to network connection structure.
 * @param pHeader - Pointer to response header structure describing content.
...
#if defined(_TRACE) || defined(RTXHTTPDBG)
/**
