...
#else
   OS_UNUSED_ARG(myCallback);
   OS_UNUSED_ARG(pStrmInfo);
#endif
}
...
static void* _getNextFreePtr (void* pMemBlk, OSFreeElemLink* pFreeLink)
{
...
static void* _getPrevFreePtr (void* pMemBlk, OSFreeElemLink* pFreeLink)
{
   OSOCTET* pMemBlkCode = (OSOCTET*)pMemBlk + OSMemBlk_code;
...
void _rtxMemPrintMemBlk (OSMemBlk* pMemBlk, const char* name) {
   ...
   if (pMemBlk->maxFreeElem == OSUINT16_MAX)
      ...
      rtxPrintToStream (g_dbg_ctxt, "%u\n",
         pMemBlk->maxFreeElem > 0 ? (pMemBlk->maxFreeElem - 1) * 8u : 0);
}
...
/**
 * Print memory block information with element detail.
 */
void _rtxMemPrintMemBlkDetail (OSMemBlk* pMemBlk, const char* name) {

   OSMemElemDescr* pElem;
   OSMemElemDescr* pElemEnd;
   _rtxMemPrintMemBlk(pMemBlk, name);
...
void _rtxMemPrintMemBlkFreeList (OSMemBlk* pMemBlk, const char* name)
{
   ...
   OSFreeElemLink* pcode = (OSFreeElemLink*) &pMemBlk->code;

   rtxPrintToStream(g_dbg_ctxt, "Mem block free elem list, ptr = %p; %s\n",
                                 pMemBlk, name);

   pFreeLinkEnd = pFreeLink = GET_FIRST_FREE (pMemBlk);

   do {
      OSFreeElemLink* pPrevFreeLink;
      OSMemElemDescr* pFreeElem;
      OSMemElemDescr* pPrevFreeElem;

      pFreeElem = GET_FREE_ELEM (pFreeLink);

      /* Get the previous free link from the current link, and
         get the element that free link is for.
      */
      pPrevFreeLink = _getPrevFreePtr(pMemBlk, pFreeLink);
      pPrevFreeElem = GET_FREE_ELEM (pPrevFreeLink);

      /*
      Output is like:
         address, prevFree @address
      I only print the previous because the next is clear from
      the full output.
      */
      if ( pFreeLink == pcode )
         rtxPrintToStream (g_dbg_ctxt, "ptr = (code), ");
      else
         rtxPrintToStream (g_dbg_ctxt, "ptr = %p, ", pFreeElem);

      if ( pPrevFreeLink == pcode )
         rtxPrintToStream (g_dbg_ctxt, "prevFree = (code)\n");
      else
         rtxPrintToStream (g_dbg_ctxt, "prevFree = %p\n",
                           pPrevFreeElem);

      pFreeLink = _getNextFreePtr (pMemBlk, pFreeLink);
   } while (pFreeLink != pFreeLinkEnd);

   rtxPrintToStream (g_dbg_ctxt, "<end free list>\n\n");
}
...
/**
 * Internal function for printing heap with detail.
 * @param ppvMemHeap Pointer to the memory heap
 * @param inclFreeList If TRUE, output should include the free memory list
 */
static void _rtxMemHeapPrintDetail (void **ppvMemHeap, OSBOOL inclFreeList);
...
{
   _rtxMemHeapPrintDetail(ppvMemHeap, FALSE);
}

void rtxMemHeapPrintWithFree (void **ppvMemHeap)
{
   _rtxMemHeapPrintDetail(ppvMemHeap, TRUE);
}

/* see description on forward decl above */
void _rtxMemHeapPrintDetail (void **ppvMemHeap, OSBOOL inclFreeList)
{
   OSMemHeap* pMemHeap;
   OSMemLink* pMemLink;
   OSMemLink* pMemLinkEnd;
   ...
   if (pMemLink) {
      do {
         ...
         _rtxMemPrintMemBlkDetail (pMemBlk, "pMemBlk");

         /* print out the free list */
         if ( inclFreeList )
         {
            _rtxMemPrintMemBlkFreeList(pMemBlk, "");
         }
