...
/* Support large files.
   Large file support ( > 2GB ) is not standardized.
*/
#ifdef _MSC_VER
   /* Windows */
   ...
   /* Newer versions of MingGW 32 allegedly have ftello/fseeko, but
      older versions only have ftello64/fseeko64 */
   #define FTELL ftello64
   #define FSEEK fseeko64
#else
   /* POSIX */
   /* gcc requires _FILE_OFFSET_BITS be defined as 64 in order for ftello and
      fseeko to have a 64-bit integer return type.
      Documenation says it must be defined before any header file is
      included.
   */
...
/* Maximum value for type off_t */
#define MAX_OFFSET 9223372036854775807 /* 2^63 - 1*/
...
static long fileRead (OSRTSTREAM* pStream, OSOCTET* pbuffer, size_t bufSize)
{
   ...
   RTLHEXDUMP (fdesc->pctxt, pbuffer, readBytes);
...
static int fileSkip (OSRTSTREAM* pStream, size_t skipBytes)
{
   ...
   /* Error if pos can't fit in off_t. */
   #if SIZE_MAX > MAX_OFFSET
      if (skipBytes > MAX_OFFSET) return RTERR_TOOBIG;
   #endif

   /* It's legal to seek past EOF, but we don't want to skip past EOF.
      So, we seek 1 less byte than we're skipping, and then do a read.
      That way, skipping past EOF will produce an error.
   */
   if (skipBytes > 1) {
      if (FSEEK (fdesc->fp, (off_t)skipBytes - 1, SEEK_CUR))
         return RTERR_READERR;
      else {
         pStream->bytesProcessed += skipBytes - 1;
      }
   }

   retval = fileRead(pStream, &data, 1);
   if (retval == 0 ) retval = RTERR_ENDOFFILE;
   else if (retval == 1) {
      retval = 0;
      pStream->bytesProcessed += 1;
   }
   /* else: retval is an error code */
...
static int fileMark (OSRTSTREAM* pStream, size_t readAheadLimit)
{
   ...
   fdesc->savedIndex =  lstat - (off_t)offset;
...
static int fileGetPos (OSRTSTREAM* pStream, size_t* ppos)
{
   ...
   savedIndex = FTELL (fdesc->fp) - (off_t)offset;
   if (savedIndex < 0) return RTERR_READERR;

   /* Error if savedIndex can't fit in *ppos. */
   if (sizeof(off_t) > sizeof(size_t) && savedIndex > SIZE_MAX)
   {
      return RTERR_TOOBIG;
   }
...
static int fileSetPos (OSRTSTREAM* pStream, size_t pos)
{
   ...
   /* Error if pos can't fit in off_t. */
   #if SIZE_MAX > MAX_OFFSET
      if (pos > MAX_OFFSET) return RTERR_TOOBIG;
   #endif

   if (FSEEK (fdesc->fp, (off_t) pos, SEEK_SET) != 0)
      return RTERR_READERR;
