...
EXTERN int asn1PD_ApproachOrLane (OSCTXT* pctxt, ApproachOrLane* pvalue)
{
   ...
   switch (ui) {
      ...
      case 0:
         ...
         stat = asn1PD_ApproachID (pctxt, (ApproachID*)&pvalue->u.approach);
      ...
      case 1:
         ...
         stat = asn1PD_LaneID (pctxt, (LaneID*)&pvalue->u.lane);
         if (stat != 0) return stat;
   ...
   for (i_ = 0; i_ < 10; i_++) {
      switch (i_) {
         ...
         if (j_ < sizeof(optbits) && optbits[j_++]) {
            ...
            pvalue->partOne = rtxMemAllocType (pctxt, BSMcoreData);

            if (pvalue->partOne == NULL)
               return RTERR_NOMEM;

            asn1Init_BSMcoreData ((BSMcoreData*)pvalue->partOne);

            stat = asn1PD_BSMcoreData (pctxt, (BSMcoreData*)pvalue->partOne);
         ...
         if (j_ < sizeof(optbits) && optbits[j_++]) {
            ...
            pvalue->path = rtxMemAllocType (pctxt, PathHistory);

            if (pvalue->path == NULL)
               return RTERR_NOMEM;

            asn1Init_PathHistory ((PathHistory*)pvalue->path);

            stat = asn1PD_PathHistory (pctxt, (PathHistory*)pvalue->path);
         ...
         pvalue->intersectionID = rtxMemAllocType (pctxt, 
            IntersectionReferenceID);

         if (pvalue->intersectionID == NULL)
            return RTERR_NOMEM;

         asn1Init_IntersectionReferenceID ((IntersectionReferenceID*)pvalue->intersectionID);

         stat = asn1PD_IntersectionReferenceID (pctxt, (IntersectionReferenceID*)pvalue->intersectionID);
         ...
         pvalue->eventFlag = rtxMemAllocType (pctxt, VehicleEventFlags);

         if (pvalue->eventFlag == NULL)
            return RTERR_NOMEM;

         asn1Init_VehicleEventFlags ((VehicleEventFlags*)pvalue->eventFlag);

         stat = asn1PD_VehicleEventFlags (pctxt, (VehicleEventFlags*)pvalue->eventFlag);
