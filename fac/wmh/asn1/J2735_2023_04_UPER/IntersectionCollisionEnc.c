...
EXTERN int asn1PE_IntersectionCollision (OSCTXT* pctxt, IntersectionCollision* pvalue)
{
   ...
   if (pvalue->m.partOnePresent) {
      stat = asn1PE_BSMcoreData (pctxt, (BSMcoreData*)pvalue->partOne);
      if (stat != 0) return stat;
   }
   ...
   if (pvalue->m.pathPresent) {
      stat = asn1PE_PathHistory (pctxt, (PathHistory*)pvalue->path);
      if (stat != 0) return stat;
   }
   ...
   stat = asn1PE_IntersectionReferenceID (pctxt, (IntersectionReferenceID*)pvalue->intersectionID);
   if (stat != 0) return stat;
   ...
   stat = asn1PE_VehicleEventFlags (pctxt, (VehicleEventFlags*)pvalue->eventFlag);
   if (stat != 0) return stat;
