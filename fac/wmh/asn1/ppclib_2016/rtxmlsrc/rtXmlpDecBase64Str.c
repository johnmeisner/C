...
EXTXMLMETHOD int rtXmlpDecBase64Str
(OSCTXT* pctxt, OSOCTET* pvalue, OSUINT32* pnocts, OSSIZE bufsize)
{
   ...
   if ( pnocts ) {
      ...
      ret = rtXmlpDecBase64Str64(pctxt, pvalue, &numocts, bufsize);
      if ( ret != 0 ) return LOG_RTERR(pctxt, ret);

      if ( numocts > OSUINT32_MAX ) return LOG_RTERR(pctxt, RTERR_TOOBIG);
      *pnocts = (OSUINT32) numocts;

      return 0;
   }
   else return rtXmlpDecBase64Str64(pctxt, pvalue, 0, bufsize);
}


EXTXMLMETHOD int rtXmlpDecBase64Str64
