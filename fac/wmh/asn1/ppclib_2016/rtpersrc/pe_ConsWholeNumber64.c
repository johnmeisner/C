...
{ 
   ...
   */
   ...
   if ( adjusted_value <= OSUINT32_MAX && range_value <= OSUINT32_MAX) {
      ...
      OSUINT32 range_bitcnt = pu_bitcnt64 (range_value - 1);
      ...
      /* If aligned, encoding depended on range value (10.5.7) */
         ...
         if ( range_value > OSINTCONST(65536) ) {
            ...
            OSUINT32 nocts = adjusted_value == 0 ? 1 : 
                                       (pu_bitcnt64 (adjusted_value) + 7) /8;

            /* len_bits will be the number of bits in the length determinant */
            OSUINT32 len_bits = pu_bitcnt64 (((range_bitcnt + 7) /8) - 1);

            stat = pe_bits (pctxt, nocts - 1, len_bits);
            if (stat != 0) return LOG_RTERR (pctxt, stat);
