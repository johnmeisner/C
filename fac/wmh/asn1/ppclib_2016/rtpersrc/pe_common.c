...
#if _MSC_VER >= 1400 /* vs 2005 and higher has this include; 2003 doesn't */
...
int pe_bits8 (OSCTXT* pctxt, OSUINT8 value, OSUINT32 nbits)
{
   size_t idx = pctxt->buffer.byteIndex;
   unsigned bitOff = pctxt->buffer.bitOffset;
...
int pe_bits8Aligned (OSCTXT* pctxt, OSUINT8 value, OSUINT32 nbits)
{
   size_t idx = pctxt->buffer.byteIndex;
   unsigned bitOff = pctxt->buffer.bitOffset;
                        ...
                        OSUINT32 adjusted_value, OSUINT32 range_value)
{
   ...
   /* If aligned, encoding depended on range value (10.5.7) */
      ...
      if (range_value <= 255) {
         return pe_bits8 (pctxt, (OSUINT8) adjusted_value, range_bitcnt);
      }

      /* If range is exactly 256, one-octet case (10.5.7b) */

      else if (range_value == 256) {
         return pe_bits8Aligned (pctxt, (OSUINT8) adjusted_value, 8);
      }
...
EXTPERMETHOD int pe_Length (OSCTXT* pctxt, OSSIZE value)
{
   ...
   if (sizeValueRange.upper < OSINTCONST(65536)) {
...
EXTPERMETHOD int pe_SmallLength (OSCTXT* pctxt, OSSIZE value)
{
   ...
   if (value <= 64) {
      ...
      if (stat != 0) return LOG_RTERR (pctxt, stat);
      stat = pe_bits8 (pctxt, (OSUINT8)(value-1), 6); /* encode n-1 */
   }
   else {
      int enclen;
...
EXTPERMETHOD int pe_UnconsLength (OSCTXT* pctxt, OSSIZE value)
{
   ...
   if (pctxt->buffer.aligned) {
      /* 1 octet case */
      if (value < 128) {
         stat = pe_bits8Aligned (pctxt, (OSUINT8) value, 8);
         enclen = (stat == 0) ? (int)value : stat;
      }
      /* 2 octet case */
      else if (value < 16384) {
      ...
      /* fragmentation case */
         ...
         stat = pe_bits8Aligned (pctxt, (OSOCTET) (multiplier | 0xC0u), 8);
      ...
      /* 1 octet case */
      if (value < 128) {
         stat = pe_bits8 (pctxt, (OSUINT8) value, 8);
         enclen = (stat == 0) ? (int)value : stat;
      }
      /* 2 octet case */
      else if (value < 16384) {
      ...
      /* fragmentation case */
         ...
         int multiplier = (value > 64*1024) ? 4 : (int)value/16384;
         stat = pe_bits8 (pctxt, (OSUINT8) (multiplier | 0xC0u), 8);
         enclen = (stat == 0) ? 16384 * multiplier : stat;
      }
   }
   if (stat != 0) return LOG_RTERR (pctxt, stat);
