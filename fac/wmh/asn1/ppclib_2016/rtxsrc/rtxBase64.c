   ...
   OSBOOL url)
{
   ...
   for (i = 0; i < srcDataSize; ) {
      if (OS_ISSPACE (pSrcData[i])) {
         ...
         if (ch[j] < 0) {
            RTDIAG4 (pctxt, "bad char: offset = %d, ch = %c (%d)\n",
         ...
         if (j < 2 || j > 3) {
            RTDIAG3 (pctxt, "invalid number of characters %d detected in "
   ...
   OSBOOL url)
{
   ...
   if (stat > 0) {
      RTHEXDUMP (pctxt, pvalue, (size_t)stat);
      if (bufsiz != (size_t)stat) {
         RTDIAG3 (pctxt, "Size mismatch: bufsiz = %lu, stat = %ld\n",
                  bufsiz, stat);
      }
   }
...
int rtxBase64toInt (char c)
{
   ...
   return (((c) < 128) ? decodeTable [(c) - 40] : -1);
}
