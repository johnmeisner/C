...
{
   OSINT32 result = rand() * 2147001325L + 715136305L;
   OSUINT32 range;
...
{
   OSUINT32 result = rand() * 2147001325L + 715136305L;
...
{
   OSREAL result = rand()/((double)(RAND_MAX)+1);
   return lower + result*(upper-lower);
}

static size_t getRandSize (OSCTXT* pctxt, size_t lower, size_t upper)
{
   if (sizeof(size_t) == 4)
      return rtxRandUInt32 (pctxt, (OSUINT32)lower, (OSUINT32)upper);
   else
      return (size_t)rtxRandUInt64 (pctxt, lower, upper);
}

char* rtxRandString (OSCTXT* pctxt, size_t lower, size_t upper,
                     const char* charSet)
{
   char *resultStr = NULL;
   size_t charSetLen = 0;
   size_t strLength = lower, iPos;

   /* check if max length 0 */
   if (0 == upper) return 0;

   /* generate length */
   strLength = getRandSize (pctxt, lower, upper);
   ...
   for (iPos = 0; iPos < strLength; iPos++)
      if (charSetLen > 0)
         resultStr[iPos]=charSet[getRandSize(pctxt, 0, charSetLen-1)];
      else
         resultStr[iPos]=(char)alphanum_str[getRandSize
...
OSUTF8CHAR* rtxRandUnicodeString (OSCTXT* pctxt, size_t lower,
...
(OSCTXT* pctxt, OSDynOctStr* pvalue, size_t lower, size_t upper)
{
   size_t strLength = lower, iPos;
   OSOCTET* p = 0;
