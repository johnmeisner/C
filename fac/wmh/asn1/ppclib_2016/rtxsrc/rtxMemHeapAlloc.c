...
EXTRTMETHOD void* rtxMemHeapSysAlloc (void** ppvMemHeap, size_t nbytes)
{
   OSMemHeap* pMemHeap;
...
EXTRTMETHOD void* rtxMemHeapAlloc (void** ppvMemHeap, size_t nbytes)
{
   ...
   if ((pMemHeap->flags & RT_MH_USEATOMBLOCK) || (nunits > (1UL<<16) - 4)) {
      ...
      OSUINT16 nunits16 = (OSUINT16) nunits;

      if (nunits16 == 0)
         nunits16 = 2;  /* nbytes == 0*/
      else
         nunits16++; /* unit for memory element record */
         ...
         Create new memBlk.  */
      {
         ...
         OSUINT32 defBlkSize = pMemHeap->defBlkSize;
         OSUINT32 dataUnits;
         OSUINT32 allocSize =
            (OSUINT32) ((nunits16 + 3) * 8u + sizeof (OSMemBlk));

         /* allocate # of bytes at least defBlkSize and ensure it is some */
         /* multiple of defBlkSize */
         allocSize = (OSUINT32) (allocSize < defBlkSize) ? defBlkSize :
            ((allocSize + defBlkSize - 1) / defBlkSize * defBlkSize);

         dataUnits = (OSUINT32) ((allocSize - sizeof (OSMemBlk)) >> 3u) + 1;

         if (dataUnits > (1ul<<16)) {
            dataUnits = (OSUINT32) (1ul<<16);
            allocSize = (OSUINT32) ((dataUnits - 1) * 8u + sizeof (OSMemBlk));
         }
         ...
         pMemBlk->nunits = (OSUINT16) (dataUnits - 1);
         pMemBlk->maxFreeElem =
            (OSUINT16) (pMemBlk->nunits - nunits16 - 2);
         ...
         if (pMemBlk->maxFreeElem == 1) {
            ...
            nextFree = (OSUINT16) (pMemBlk->nunits - 1);

         pMemBlk->firstFree = nextFree;
         pMemBlk->nsaved = 0;
         pMemBlk->code = OSMEMLINKCODE;
         ...
         */
         ...
         if (pMemBlk->maxFreeElem > 0) {
            /* free space */
            nunits16 = pMemBlk->nunits - nunits16 - 2;
            SET_OSUINT16 (pElem, OSMemElemDescr_nunits, nunits16);
            SET_FREE (pElem);

            /* GET_FREE_LINK returns a pointer to the last unit associated
            with pElem, which holds the OSFreeElemLink structure.  */
            pFreeLink = GET_FREE_LINK (pElem);

            if (0 != pFreeLink) {
               /*  There are no free elements before or after this one. This
               ...
               /* Inconsistency bw free link and maxFreeElem: we'll treat this
                * just like having no memory left in the block */
               RTMEMDIAG1 (pMemHeap, "~E rtxMemHeapAlloc: failed; pMemBlk "
                     "reports a free link, but the link is null\n");
...
EXTRTMETHOD void* rtxMemHeapAlloc (void** ppvMemHeap, size_t nbytes)
{
   ...
   pElem = GET_ELEM (mem_p);
   pElem_flags (pElem) = 0;
   pElem_code (pElem) = OSMEMLINKCODE;
   pElem_refcnt (pElem) = 1;
