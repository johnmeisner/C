...
EXTRTMETHOD ASN1CBitStr::ASN1CBitStr() :
   ...
   mUnitsUsed (0), mUnitsAllocated (0), mDynAlloc (FALSE)
{
   mpNumBits = new ASN1CBitStrSizeHolder64 (_numbits);
}
...
EXTRTMETHOD ASN1CBitStr::ASN1CBitStr(OSOCTET* bitStr,
   OSSIZE& octsNumbits, OSSIZE maxNumbits_):
...
void ASN1CBitStr::privateInit (OSOCTET* bitStr, OSSIZE maxNumbits_)
{
   mMaxNumBits = maxNumbits_;
   (*mpUnits) = bitStr;

   if (0 != mpNumBits) {
      if (mMaxNumBits < mpNumBits->getValue()) {
   ...
   ASN1CType(bitStr), mpUnits(&_units)
{
   ...
   if(mUnitsAllocated) {
      ...
      (*mpUnits) = allocateMemory(mUnitsAllocated);
      OSCRTLSAFEMEMCPY (*mpUnits, mUnitsAllocated, *bitStr.mpUnits, mUnitsUsed * sizeof(OSOCTET));
   }
   else {
      (*mpUnits) = 0;
      mUnitsUsed = 0;
