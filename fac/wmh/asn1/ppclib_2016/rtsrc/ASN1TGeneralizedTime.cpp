...
#ifdef OMNITRACS_RT
   struct tm loctm = *rtWCELocalTime(&time);
#else
   struct tm loctm = *localtime(&time);
#endif
   if (diffTime || mbDerRules) {
#ifdef OMNITRACS_RT
      struct tm gmtm = *rtWCEGmtTime(&time);
#else
...
EXTRTMETHOD int ASN1TGeneralizedTime::compileString (char* pbuf, size_t bufsize) const {
       ...
       (mSecFraction > 0 && mSecFracLen > 0))
   {
      ...
      if (bufsize < cursize)
         return RTERR_BUFOVFLW;
      os_snprintf(p, 3, "%.2d", minute_);
      p += 2;
      if (mbDerRules || mSecond > 0 || (mSecFraction > 0 && mSecFracLen > 0)) {
         cursize += 2;
         if (bufsize < cursize)
            return RTERR_BUFOVFLW;
         os_snprintf(p, 3, "%.2d", mSecond);
         p += 2;
   ...
   if (mbDerRules || mbUtcFlag) {
      ...
      *p++ = (mDiffHour > 0) ? '+' : '-';
      if (mDiffMin)
         os_snprintf(p, 5, "%.2d%.2d", abs(mDiffHour), abs(mDiffMin));
      else
         os_snprintf(p, 3, "%.2d", abs(mDiffHour));
   }
   return 0;
}
