...
{
    /* DENSO patch to attempt addressing race condition */
    unsigned int skerraddr = (unsigned int)sk->sk_error_report;
    /* end DENSO */
	...
	if (!sock_flag(sk, SOCK_DEAD))
        /* DENSO patch */
        if (likely(skerraddr == (unsigned int)sk->sk_error_report))
        /* END DENSO */
		sk->sk_error_report(sk);
	return 0;
}
