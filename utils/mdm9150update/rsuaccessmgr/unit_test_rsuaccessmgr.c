...
/* lpwrmonitor.c */
extern int32_t rsuaccessmgr_main(int32_t argc, char_t *argv[]);
...
void test_main(void)
{
  ...
  my_argc=2;
  strcpy(token[0],"./rsuaccessmgr");

  /* 
   * Test daemon mode which has rsudiagnostic support.
   */
  strcpy(token[1],"-d");
  rsuaccessmgr_main(my_argc,token); /* Nominal case */

 
  strcpy(token[1],"-d");
  rsuaccessmgr_main(my_argc,token); /* Fail to open device. */
